...
public class TemporalFunctionLibrary {
	...
	public static Val weekOfYear(@Text Val isoDateTime) {
		try {
			return Val.of(
					DateTimeFormatter.ISO_DATE_TIME.parse(isoDateTime.getText()).get(ChronoField.ALIGNED_WEEK_OF_YEAR));
		}
		catch (DateTimeException e) {
			return Val.error(PARAMETER_NOT_AN_ISO_8601_STRING, e.getMessage());
		}
	...
	public static Val dayOfWeek(@Text Val isoDateTime) {
		try {
			return Val.of(
					DayOfWeek.from(DateTimeFormatter.ISO_DATE_TIME.parse(isoDateTime.getText(), LocalDateTime::from))
							.toString());
		}
		catch (DateTimeException e) {
			return Val.error(PARAMETER_NOT_AN_ISO_8601_STRING, e.getMessage());
	...
	public static Val atZone(@Text Val isoDateTime, @Text Val zoneId) {
		try {
			...
			return Val.of(zonedDateTime.withZoneSameInstant(zone).toLocalDateTime().truncatedTo(ChronoUnit.SECONDS)
					.toString());
		}
	...
	public static Val localDateTime(@Text Val isoDateTime) {
		try {
			return Val.of(DateTimeFormatter.ISO_DATE_TIME.parse(isoDateTime.getText(), LocalDateTime::from)
					.truncatedTo(ChronoUnit.SECONDS).toString());
		}
		catch (DateTimeException e) {
			return Val.error(PARAMETER_NOT_AN_ISO_8601_STRING, e.getMessage());
	...
	public static void requireValue(Val nodeNotToBeNull, java.util.function.Function<Val, Boolean> validator) {
		if (nodeNotToBeNull == null || nodeNotToBeNull.isNull() || nodeNotToBeNull.isUndefined()
				|| nodeNotToBeNull.get().isNull())
	...
	private static Instant nodeToInstant(Val time) {
		if (time.isNull() || time.isUndefined())
			throw new DateTimeException("provided time value is null or undefined");
