...
public class TemporalFunctionLibrary {
    ...
    @Function(docs = "Parses the given string as local date time (ISO) and converts it to the respective time in UTC.")
    public static Val localToIso(@Text Val localDateTime) {
    ...
    @Function(docs = "Parses the given string as local date time (DIN) and converts it to the respective time in UTC.")
    public static Val dinToIso(@Text Val dinDateTime) {
    ...
    @Function(docs = "Parses the given string as an ISO date time with explicit time zone and converts it to the respective date time in UTC.")
    public static Val dateInZone(@Text Val isoDateTime) {
        var zonedDateTime = DateTimeFormatter.ISO_DATE_TIME.parse(isoDateTime.getText(), ZonedDateTime::from);
        return Val.of(
                zonedDateTime.withZoneSameInstant(ZoneId.of("UTC")).toLocalDateTime().truncatedTo(ChronoUnit.SECONDS).toString());
    }

    @Function(docs = "Parses the given string as an ISO date time with explicit offset and converts it to the respective date time in UTC.")
    public static Val dateAtOffset(@Text Val isoDateTime) {
        var offsetDateTime = DateTimeFormatter.ISO_DATE_TIME.parse(isoDateTime.getText(), OffsetDateTime::from);
        return Val.of(offsetDateTime.withOffsetSameInstant(ZoneOffset.UTC).toLocalDateTime().truncatedTo(ChronoUnit.SECONDS)
                .toString());
    }

    // ""12:12:21,345324", "+05:00"
    @Function(docs = "Parses the given string as local time and converts it to the local time at the given offset.")
    public static Val localTimeAtOffset(@Text Val localTime, @Text Val offsetId) {
        ...
        OffsetTime offsetTime = OffsetTime.of(lt, offset);

        return Val.of(offsetTime.toLocalTime().toString());
    }

    @Function(docs = "Parses the string as local time and converts it to the local time in the given zone.")
    public static Val localTimeAtZone(@Text Val localTime, @Text Val zoneId) {
        var zone = zoneIdOf(zoneId);
        LocalTime lt = DateTimeFormatter.ISO_LOCAL_TIME.parse(localTime.getText(), LocalTime::from);
        ZonedDateTime zonedDateTime = ZonedDateTime.of(lt.atDate(LocalDate.now()), zone);

        return Val.of(zonedDateTime.toLocalTime().toString());
    }

    @Function(docs = "Parses the given string as ISO time and converts it to the respective time in UTC.")
    public static Val timeAtOffset(@Text Val isoTime) {
    ...
    // "08:30am", "09:30pm"
    @Function(docs = "Parses the given string as local time in AM/PM-format and converts it to the respective time in UTC.")
    public static Val timeAMPM(@Text Val timeInAMPM) {
        LocalTime lt = DateTimeFormatter.ISO_TIME.withLocale(Locale.ENGLISH).parse(timeInAMPM.getText(), LocalTime::from);
        ZonedDateTime zonedDateTime = ZonedDateTime.of(lt.atDate(LocalDate.now()), ZoneId.systemDefault());
        return Val.of(zonedDateTime.withZoneSameInstant(ZoneId.of("UTC")).toLocalTime().toString());
    }
