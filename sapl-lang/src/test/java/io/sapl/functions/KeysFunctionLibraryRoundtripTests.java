...
class KeysFunctionLibraryRoundtripTests {
    ...
    void rsaKey_roundTripConversion_producesEquivalentKey() throws Exception {
        ...
        // Convert back to PEM
        val pemResult = KeysFunctionLibrary.publicKeyFromJwk(jwkResult);
        assertThat(pemResult.isDefined()).isTrue();

        // Parse both original and converted keys
        val originalKey  = parsePublicKeyFromPem(rsaPublicKeyPem);
        val convertedKey = parsePublicKeyFromPem(pemResult.getText());
    ...
    void ecKey_roundTripConversion_producesEquivalentKey(String pemKey, KeyPair keyPair, String jwkCurve,
        ...
        assertThat(jwkResult.isDefined()).isTrue();
        assertThat(jwkResult.get().get("crv").asText()).isEqualTo(jwkCurve);
    ...
    void rsaJwk_followsRfc7517Structure() throws Exception {
        ...
        assertThat(jwk.has("kty")).as("JWK must have 'kty' field").isTrue();
        assertThat(jwk.get("kty").asText()).isEqualTo("RSA");

        // Verify RSA-specific fields per RFC 7518 Section 6.3
        assertThat(jwk.has("n")).as("RSA JWK must have 'n' (modulus)").isTrue();
        assertThat(jwk.has("e")).as("RSA JWK must have 'e' (exponent)").isTrue();

        // Verify base64url encoding (no padding)
        val nValue = jwk.get("n").asText();
        val eValue = jwk.get("e").asText();
    ...
    void ecJwk_followsRfc7517Structure() throws Exception {
        ...
        assertThat(jwk.has("crv")).as("EC JWK must have 'crv' (curve)").isTrue();
        assertThat(jwk.has("x")).as("EC JWK must have 'x' coordinate").isTrue();
        assertThat(jwk.has("y")).as("EC JWK must have 'y' coordinate").isTrue();

        // Verify curve name follows RFC 7518 (P-256, not secp256r1)
        assertThat(jwk.get("crv").asText()).isEqualTo("P-256");

        // Verify base64url encoding (no padding)
        val xValue = jwk.get("x").asText();
        val yValue = jwk.get("y").asText();
    ...
    void ed25519Jwk_followsRfc8037Structure() throws Exception {
        ...
        assertThat(jwk.has("crv")).as("OKP JWK must have 'crv' (curve)").isTrue();
        assertThat(jwk.get("crv").asText()).isEqualTo("Ed25519");

        assertThat(jwk.has("x")).as("OKP JWK must have 'x' parameter").isTrue();
    ...
    void ecJwk_canBeReconstructedIntoFunctionalKey() throws Exception {
        ...
        val xBytes = Base64.getUrlDecoder().decode(jwk.get("x").asText());
        val yBytes = Base64.getUrlDecoder().decode(jwk.get("y").asText());
    ...
    void ecKey_coordinates_haveCorrectLength() throws Exception {
        ...
        val yBytes = Base64.getUrlDecoder().decode(jwk.get().get("y").asText());
