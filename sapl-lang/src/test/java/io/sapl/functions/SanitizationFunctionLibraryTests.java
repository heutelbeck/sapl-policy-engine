...
class SanitizationFunctionLibraryTests {
    ...
    static Stream<Arguments> balancedModeLegitimateInputs() {
        return Stream.of(
                ...
                Arguments.of("123456", "Pure numeric"),
                Arguments.of("αβγδ", "Greek characters"),
    ...
    static Stream<Arguments> balancedModeInjectionAttempts() {
        return Stream.of(
                ...
                Arguments.of("user'-- ", "Comment with space"),
                Arguments.of("Cthulhu'#", "Hash-style comment"),
                ...
                Arguments.of("1 AND 1=1", "Boolean true condition"),
                Arguments.of("1 OR 2=2", "Boolean OR condition"),
                ...
                Arguments.of("0x41646d696e", "Hex-encoded 'Admin'"),
                Arguments.of("0x44524f50", "Hex-encoded 'DROP'"),
                Arguments.of("user_0x27", "Hex with underscore"),
                Arguments.of("%55%4e%49%4f%4e%20%53%45%4c%45%43%54%20%2a%20%46%52%4f%4d%20%75%73%65%72%73", "Hex-encoded UNION SELECT"),
                ...
                Arguments.of("\uFF33\uFF25\uFF2C\uFF25\uFF23\uFF34 * \uFF26\uFF32\uFF2F\uFF2D outer_gods", "Unicode fullwidth SELECT"),
                Arguments.of("\uFF24\uFF32\uFF2F\uFF30 \uFF34\uFF21\uFF22\uFF2C\uFF25 servitors", "Unicode fullwidth DROP"),
                ...
                Arguments.of("Select * from table where name < 'test-1' and date > 12-12-2000", "Complex SELECT with comparison")
        );
    }
    ...
    static Stream<Arguments> strictModeValidIdentifiers() {
        return Stream.of(
                ...
                Arguments.of("SHOGGOTH9000", "Entity identifier"),
                Arguments.of("Zone42", "Numeric zone"),

                // With allowed separators (hyphen, underscore are not SQL syntax)
                Arguments.of("deep-one-alpha", "Hyphenated identifier"),
                Arguments.of("SITE_19", "Underscore identifier"),
                Arguments.of("Cthulhu-Spawn-7", "Multi-hyphen code"),

                // Country/region codes
                Arguments.of("US", "Country code"),
                Arguments.of("GB", "Another country code"),
    ...
    static Stream<Arguments> strictModeRejectedInputs() {
        return Stream.of(
                ...
                Arguments.of("O'Brien", "Apostrophe in surname"),
                Arguments.of("It's", "Contraction"),
                Arguments.of("R'lyeh", "Apostrophe in location name"),

                // Semicolons
                Arguments.of("Item1; Item2", "Semicolon separator"),
                Arguments.of("Test;", "Trailing semicolon"),

                // SQL keywords
                Arguments.of("SELECT", "SQL keyword"),
                Arguments.of("UserOrAdmin", "Contains OR keyword"),
                Arguments.of("Portland OR Seattle", "City names with OR"),
                Arguments.of("Anderson", "Name containing AND"),

                // SQL metacharacters
                Arguments.of("user(*)", "Parentheses and asterisk"),
                Arguments.of("value*2", "Multiplication operator"),
                Arguments.of("func()", "Function call syntax"),

                // Comments
                Arguments.of("test--comment", "Double dash comment"),
                Arguments.of("value#note", "Hash comment"),

                // Encoded characters
                Arguments.of("%27", "URL-encoded apostrophe"),
                Arguments.of("0x41", "Hex-encoded value"),
                ...
                Arguments.of("' OR '1'='1", "Classic injection"),
                Arguments.of("1; DROP TABLE", "Stacked query")
        );
    }
    ...
    static Stream<Arguments> balancedVsStrictComparison() {
        return Stream.of(
                Arguments.of("O'Brien", false, true, "Name with apostrophe"),
    ...
    void compareModes_behaviorDifferences(String input, boolean balancedBlocks, boolean strictBlocks, String description) {
        final var inputVal = Val.of(input);
        ...
        if (balancedBlocks) {
            ...
            assertThatVal(balancedResult).as("Balanced mode should allow: " + description)
                    .isEqualTo(inputVal);
        }
        ...
        if (strictBlocks) {
            ...
            assertThatVal(strictResult).as("Strict mode should allow: " + description)
                    .isEqualTo(inputVal);
        }
    }
