...
public class KMLTest {
	...
	assertEquals("KML file is not correctly imported into GeoPIPResponse",
		kml.toGeoPIPResponse().toString(), EXPECTED_RESPONSE);
    }
	...
	assertEquals("Different constructors result in different import configurations.", stringKml,
		jsonKml);
    }
	...
	try {
	    ...
	    assertEquals("Wrong exception thrown when trying to access a non existing KML-file.",
		    e.getMessage(), KMLImport.UNABLE_TO_PARSE_KML);
	}
    }
