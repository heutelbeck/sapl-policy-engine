...
- `target/sapl-language-server-4.1.0.jar` - Library JAR for Maven dependencies
- `target/sapl-language-server-4.1.0-standalone.jar` - Executable JAR with all dependencies
...
```shell
java -agentlib:native-image-agent=security-output-dir=native-security -jar target/sapl-language-server-4.1.0-standalone.jar
```
...
```shell
java -jar sapl-language-server-4.1.0-standalone.jar
```
...
```shell
java -jar sapl-language-server-4.1.0-standalone.jar --socket --port=5007
```

### Debug Logging

```shell
java -Dorg.slf4j.simpleLogger.defaultLogLevel=DEBUG -jar sapl-language-server-4.1.0-standalone.jar
```
