 ...
 * in SAPL policy evaluation. Attributes
 * can be published with time-to-live settings and timeout strategies.
 * <p>
 * Attributes are identified by a composite key consisting of entity, name, and
 * arguments. The entity parameter
 * distinguishes between global attributes (entity is null) and entity-specific
 * attributes (entity has a value). Global
 * attributes are shared across all policy evaluations, while entity-specific
 * attributes are scoped to particular
 * entities like users, devices, or sessions.
 * <p>
...
public interface AttributeRepository extends AttributeFinder {
    ...
    enum TimeOutStrategy {
         ...
         * ATTRIBUTE_UNAVAILABLE after removal.
         */
        REMOVE,

        /**
         * Keeps the attribute but sets its value to undefined when TTL expires.
         * Subscribers will receive Val.UNDEFINED
         * after expiration.
         */
        BECOME_UNDEFINED
    }
     ...
     * timeout behavior. This is the primary
     * method that all convenience methods delegate to.
     * <p>
     * The entity parameter determines attribute scope. When entity is null, the
     * attribute is global and shared across
     * all policy evaluations. When entity has a value, the attribute is scoped to
     * that specific entity. Two attributes
     ...
     * are safe for concurrent access from
     * multiple threads.
     *
     * @param entity
     * the entity context for this attribute, or null for global attributes
     * @param attributeName
     * the fully qualified attribute name
     * @param arguments
     * the attribute arguments, or empty list if none
     * @param value
     * the attribute value to publish
     * @param ttl
     * time-to-live duration after which the timeout strategy is applied
    ...
    /**
     * Publishes a global attribute with infinite TTL and removal strategy. Suitable
     * for static configuration values or
    ...
    /**
     * Publishes a global attribute with specified TTL and removal strategy. When
     * TTL expires, the attribute is removed
     * from the repository.
     * <p>
     * Example:
     *
    ...
    /**
     * Publishes a global attribute with specified TTL and timeout strategy. Allows
     * control over whether the attribute
     * is removed or becomes undefined when TTL expires.
     * <p>
     * Example:
     *
     * <pre>{@code
    ...
    /**
     * Publishes an entity-specific attribute with infinite TTL. Useful for
     * associating attributes with specific
     * entities like users or resources.
     * <p>
     * Example:
     *
     * <pre>{@code
     ...
     *
     * @param entity
     * the entity context for this attribute
     * @param attributeName
     * the fully qualified attribute name
     * @param value
     * the attribute value to publish
    ...
    /**
     * Publishes an entity-specific attribute with specified TTL. The attribute is
     ...
     * Provides full control over entity-bound
    ...
    /**
     * Publishes a global attribute with arguments and infinite TTL. Useful for
     * parameterized attributes like computed
    ...
    /**
     * Publishes a global attribute with arguments and specified TTL. The attribute
     * is removed when TTL expires.
     * <p>
     * Example:
     *
     * <pre>{@code
     ...
     * Provides full control for parameterized
     * global attributes.
     * <p>
     * Example:
     *
     ...
     * ATTRIBUTE_UNAVAILABLE. The attribute key is
     * determined by the combination of entity, name, and arguments.
     *
     * @param entity
     * the entity context, or null for global attributes
     * @param attributeName
     * the fully qualified attribute name
     * @param arguments
     * the attribute arguments, or empty list if none
