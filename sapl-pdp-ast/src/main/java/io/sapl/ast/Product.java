 ...
 */
public record Product(@NonNull List<Expression> operands, @NonNull Nature nature, @NonNull SourceLocation location)
        implements Expression {

    public Product {
        if (operands.size() < 3) {
            throw new SaplCompilerException("Product requires at least 3 operands, use BinaryOperation for 2",
                    location);
        }
