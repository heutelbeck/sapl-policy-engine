...
public class QueryCreation {
	...
	public static <T> String manipulateQuery(String basicQuery, ArrayNode conditions, ArrayNode selections,
		...
		return QuerySelectionUtils.createSelectionPartForAnnotation(finalQuery, selections, domainType);
	}
	...
	public static String createBaselineQuery(MethodInvocation invocation) {
		...
		var sortingPart = ConvertToSQL.prepareAndMergeSortObjects(Sort.unsorted(), invocation.getArguments());
	 ...
	 */
	public <T> String createSqlQuery(ArrayNode conditions, ArrayNode selections, Class<T> domainType,
			String baseQuery) {
		var queryBuilder = new StringBuilder();

		queryBuilder.append(QuerySelectionUtils.createSelectionPartForMethodNameQuery(selections, domainType));
