...
public class R2dbcAnnotationQueryManipulationEnforcementPoint<T> {
	...
	private Function<AuthorizationDecision, Flux<T>> enforceDecision(Class<T> domainType, MethodInvocation invocation) {
		...
		return decision -> {
			...
			if (decisionIsPermit) {
				var bundle = constraintQueryEnforcementServiceProvider.getObject().queryManipulationBundelFor(decision,
						true);

				var conditions = bundle.getConditions();
				var selections = bundle.getSelections();

				return retrieveData(conditions, selections, baseQuery, domainType);
			} else {
	 ...
	 *         query.
	 */
	private Flux<T> retrieveData(ArrayNode conditions, ArrayNode selections, String basicQuery, Class<T> domainType) {

		var manipulatedCondition = QueryCreation.manipulateQuery(basicQuery, conditions, selections, domainType);
