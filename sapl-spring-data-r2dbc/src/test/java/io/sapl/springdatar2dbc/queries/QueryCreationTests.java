...
class QueryCreationTests {
	...
	void when_manipulateQuery_then_returnManipulatedQueryContainsWhere1() {
		...
		querySelectionUtilsMock.when(() -> QuerySelectionUtils.createSelectionPartForAnnotation(anyString(),
				any(ArrayNode.class), eq(Person.class))).thenReturn(expected);

		var result = QueryCreation.manipulateQuery(baseQuery, CONDITIONS_ONE, SELECTIONS, Person.class);
		
		// THEN
		assertEquals(result, expected);
		
		querySelectionUtilsMock.verify(() -> QuerySelectionUtils.createSelectionPartForAnnotation(anyString(), any(ArrayNode.class), eq(Person.class)), times(1));
	}
	...
	void when_manipulateQuery_then_returnManipulatedQueryContainsWhere3() {
		...
		var result = QueryCreation.manipulateQuery(baseQuery, CONDITIONS_WITH_AND, SELECTIONS, Person.class);
	...
	void when_manipulateQuery_then_returnManipulatedQueryContainsNoWhere() {
		...
		var result = QueryCreation.manipulateQuery(baseQuery, CONDITIONS_WITH_OR, SELECTIONS, Person.class);
	...
	void when_createBaselineQuery_then_returnBaselineQuery() {
		...
		// WHEN
		queryAnnotationParameterResolverMock.when(() -> QueryAnnotationParameterResolver.resolveForRelationalDatabase(any(Method.class),
				any(Object[].class))).thenReturn(baseQuery);
		convertToSQLMock.when(() -> ConvertToSQL.prepareAndMergeSortObjects(any(Sort.class), any(Object[].class))).thenReturn(sortingPart);
		...
		queryAnnotationParameterResolverMock.verify(() -> QueryAnnotationParameterResolver.resolveForRelationalDatabase(any(Method.class),
				any(Object[].class)), times(1));
		convertToSQLMock.verify(() -> ConvertToSQL.prepareAndMergeSortObjects(any(Sort.class), any(Object[].class)), times(1));
	}
	...
	void when_createSqlQuery_then_returnSqlQuery1() {
		...
		// WHEN
		querySelectionUtilsMock.when(() -> QuerySelectionUtils.createSelectionPartForMethodNameQuery(any(ArrayNode.class), eq(Person.class))).thenReturn(selectionPart);

		var result = QueryCreation.createSqlQuery(CONDITIONS_ONE, SELECTIONS, Person.class, baseQuery);
		
		// THEN
		assertEquals(result, expected);
		
		querySelectionUtilsMock.verify(() -> QuerySelectionUtils.createSelectionPartForMethodNameQuery(any(ArrayNode.class), eq(Person.class)), times(1));
	}
	...
	void when_createSqlQuery_then_returnSqlQuery3() {
		...
		var result = QueryCreation.createSqlQuery(EMPTY_ARRAYNODE, SELECTIONS, Person.class, baseQuery);
	...
	void when_createSqlQuery_then_returnSqlQuery4() {
		...
		var result = QueryCreation.createSqlQuery(CONDITIONS_TWO, SELECTIONS, Person.class, baseQuery);
