...
class R2dbcAnnotationQueryManipulationEnforcementPointTests {
	...
	QueryManipulationExecutor queryManipulationExecutorMock = mock(QueryManipulationExecutor.class);
	ConstraintQueryEnforcementService constraintQueryEnforcementServiceMock = mock(
			ConstraintQueryEnforcementService.class);
	QueryManipulationConstraintHandlerBundle queryManipulationConstraintHandlerBundleMock = mock(
	...
	public void beforeEach() throws InitializationException {
		...
		lenient().when(objectProviderQueryManipulationExecutorMock.getObject()).thenReturn(queryManipulationExecutorMock);
		...
		lenient().when(queryManipulationExecutorMock.execute(anyString(), eq(Person.class))).thenReturn(Flux.just(cathrin));
		...
		queryCreationMock.when(
				() -> QueryCreation.manipulateQuery(anyString(), any(ArrayNode.class), any(ArrayNode.class), eq(Person.class)))
				.thenReturn(baseQuery);
	}
	...
	void when_enforce_then_returnFluxDomainObject() {
		...
		when(constraintQueryEnforcementServiceMock.queryManipulationBundelFor(any(AuthorizationDecision.class),
				any(Boolean.class))).thenReturn(queryManipulationConstraintHandlerBundleMock);
