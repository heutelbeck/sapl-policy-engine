...
/**
 * Multi requests return a {@link Flux flux} of {@link IdentifiableDecision identifiable decisions} or
 * {@link IdentifiableResponse identifiable responses}. The blocking implementation should collect the
 * emitted decisions / responses and return a {@link MultiDecision} / {@link MultiResponse} as soon as
 * at least one decision / response for each request contained in the {@link MultiRequest} has been
 * collected. This class provides according helper methods.
 */
public class BlockingMultiRequestSupport {
    ...
    public static MultiDecision collectDecisions(MultiRequest multiRequest, Flux<IdentifiableDecision> decisionFlux) {
        ...
        final Disposable subscription = decisionFlux
                .subscribe(
                        identifiableDecision -> {
                            // collect the decisions and wait until at least one decision has arrived for each request
    ...
    public static MultiResponse collectResponses(MultiRequest multiRequest, Flux<IdentifiableResponse> responseFlux) {
        ...
        final Disposable subscription = responseFlux
                .subscribe(
