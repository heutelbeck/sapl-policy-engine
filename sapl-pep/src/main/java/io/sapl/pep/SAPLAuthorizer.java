...
public class SAPLAuthorizer {
	...
	/**
	 * Retrieves authorization responses from the policy decision point but does not handle any obligations
	 * or advice contained in the responses (only checks, whether obligations could be handled). If the
	 * authorization decision is {@link Decision#PERMIT} and handlers for all the obligations contained in the
	 * response have been registered with the current obligation handler, {@link Decision#PERMIT} is pushed to
	 * the returned {@link Flux}, {@link Decision#DENY} otherwise.<br>
	 * This method is meant to be used if the specified action should not yet be executed, but the caller still
	 * has to know whether it could be executed (e.g. to decide whether a link or button triggering the action
	 * should be displayed/enabled).<br>
	 ...
	 *          of another class using an available {@link SaplMapper SaplMapper} before sending it to the
	 *          policy decision point.
     * @return a {@link Flux} emitting the decisions of the responses for the given request. Decisions are only
	...
	/**
	 * Multi-request variant of {@link #wouldAuthorize(Object, Object, Object, Object)}.
     *
	 ...
	 * @return a {@link Flux} emitting the decisions of the responses for the given requests. Related decisions
	 *         and requests have the same id.
	 */
	...
	/**
	 * Retrieves authorization responses from the policy decision point and handles the obligations and
	 * advice if needed. The final decisions are returned as a {@link Flux}.
	 *
	 * @param subject
	 * 			an object representing the subject of the authorization request. May be mapped to an instance
	 *          of another class using an available {@link SaplMapper SaplMapper} before sending it to the
	...
	/**
	 * Retrieves authorization responses from the policy decision point and handles the obligations and
	 * advice if needed. A {@link Flux} of responses containing the current authorization decisions is
	 * returned.
	 *
	 * @param subject
	 * 			an object representing the subject of the authorization request. May be mapped to an instance
	 ...
	 *          of another class using an available {@link SaplMapper SaplMapper} before sending it to the
	 *          policy decision point.
     * @return a {@link Flux} emitting the responses for the given request. Responses are only emitted if they
	 *         are different from the preceding one.
     */
     ...
     * @param multiRequest the multi request object containing the subjects, actions, resources and environments
     *                     of the authorization requests to be sent to the PDP.
     * @return a {@link Flux} emitting the responses for the given requests. Related responses and requests have
	 *         the same id.
     */
