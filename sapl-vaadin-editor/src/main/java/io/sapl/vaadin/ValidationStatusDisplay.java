...
public class ValidationStatusDisplay extends VerticalLayout {
     ...
     *
     * @param issues list of validation issues (may be empty for success)
     */
    ...
    private void showErrors(long errorCount, long warningCount) {
        ...
        if (errorCount == 1 && warningCount == 0) {
            var firstError = currentIssues.stream().filter(i -> IssueSeverity.ERROR == i.getSeverity()).findFirst();
    ...
    private void showWarnings(long warningCount) {
        ...
        if (warningCount == 1) {
            var firstWarning = currentIssues.stream().filter(i -> IssueSeverity.WARNING == i.getSeverity()).findFirst();
     ...
     *
     * @param expanded true to show details, false to hide
     */
