...
class SaplMqttClientConnectionIT {
	...
	void when_connectionIsShared_then_bothMessageFluxWorking() throws InitializationException {
		...
		var saplMqttMessageFluxFirst  = saplMqttClient.buildSaplMqttMessageFlux(Val.of("topic1"),
				buildVariables());
		var saplMqttMessageFluxSecond = saplMqttClient.buildSaplMqttMessageFlux(Val.of("topic2"),
				buildVariables());
	...
	void when_brokerConnectionLost_then_reconnectToBroker()
		...
		var mqttClient          = startClient();
		var saplMqttMessageFlux = saplMqttClient.buildSaplMqttMessageFlux(TOPIC,
				buildVariables());

		// THEN
		StepVerifier.create(saplMqttMessageFlux)
				.thenAwait(Duration.ofMillis(1 * DELAY_MS))
				.then(() -> mqttClient.publish(MESSAGE))
				.expectNext(Val.of("message"))

				.then(mqttClient::disconnect)
				.thenAwait(Duration.ofMillis(1 * DELAY_MS))

				.then(() -> stopBroker(mqttBroker))
				.thenAwait(Duration.ofMillis(5 * DELAY_MS))
				.then(() -> startBroker(secondaryBroker))
				.thenAwait(Duration.ofMillis(2 * DELAY_MS))

				.then(mqttClient::connect)
				.thenAwait(Duration.ofMillis(5 * DELAY_MS))
