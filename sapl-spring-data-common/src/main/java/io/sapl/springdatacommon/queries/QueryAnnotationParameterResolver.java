...
public class QueryAnnotationParameterResolver {
    ...
    public static String resolveForMongoDB(Method method, Object[] args) {
        ...
        var query = String.format("%sXXXXX%sXXXXX%s", queryAnnotation.value(), queryAnnotation.fields(), queryAnnotation.sort());
        var parameterNames = getParameterNames(method);
        var finalArgs = convertArgumentsToString(args);
    ...
    private static String replaceParameters(String query, List<String> parameterNames, List<String> arguments) {
    ...
    private static String replaceParametersRelational(String query, List<String> parameterNames, List<String> arguments) {
        for (int i = 0; i < parameterNames.size(); i++) {
