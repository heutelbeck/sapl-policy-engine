...
public class QueryEnforceAuthorizationSubscriptionService {
	...
	private AuthorizationSubscription enforceAnnotationValueToAuthorizationSubscription(QueryEnforce enforceAnnotation,
		...
		var action = enforceAnnotationValueResolver(enforceAnnotation.action(), enforceAnnotation.staticClasses(),
				methodInvocation);
		var resource = enforceAnnotationValueResolver(enforceAnnotation.resource(), enforceAnnotation.staticClasses(),
				methodInvocation);
	...
	private Object enforceAnnotationValueResolver(String annotationValue, Class<?>[] staticClasses,
		...
		if (annotationValue.startsWith("#")) {
			return parseMethodParameterInEvaluationContext(annotationValue, methodInvocation);
		}
