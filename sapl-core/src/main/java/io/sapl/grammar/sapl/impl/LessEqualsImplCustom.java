...
public class LessEqualsImplCustom extends io.sapl.grammar.sapl.impl.LessEqualsImpl {
	...
	private static final int HASH_PRIME_08 = 43;
	...
	public Flux<Optional<JsonNode>> evaluate(EvaluationContext ctx, boolean isBody, Optional<JsonNode> relativeNode) {
		...
		return Flux.combineLatest(left, right, this::lessOrEqual).distinctUntilChanged();
	}

	private Optional<JsonNode> lessOrEqual(Optional<JsonNode> left, Optional<JsonNode> right) {
		try {
	...
	public int hash(Map<String, String> imports) {
		...
		hash = HASH_PRIME_08 * hash + Objects.hashCode(getClass().getTypeName());
		hash = HASH_PRIME_08 * hash + ((getLeft() == null) ? 0 : getLeft().hash(imports));
		hash = HASH_PRIME_08 * hash + ((getRight() == null) ? 0 : getRight().hash(imports));
		return hash;
	}
	...
	public boolean isEqualTo(EObject other, Map<String, String> otherImports, Map<String, String> imports) {
		...
		final LessEqualsImplCustom otherImpl = (LessEqualsImplCustom) other;
