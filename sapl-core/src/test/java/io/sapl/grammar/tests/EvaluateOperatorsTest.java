...
public class EvaluateOperatorsTest {
	...
	public void evaluateAndLeftFalseWrongDatatypeRight() {
		...
		and.evaluate(ctx, true, null).take(1).subscribe(result -> assertEquals(
	...
	public void evaluateOrWrongDatatypeRightLeftTrue() {
		...
		or.evaluate(ctx, true, null).take(1).subscribe(result -> assertEquals(
	...
	private Flux<Optional<JsonNode>> more(BigDecimal leftNumber, BigDecimal rightNumber) {
	...
	private Flux<Optional<JsonNode>> less(BigDecimal leftNumber, BigDecimal rightNumber) {
	...
	private Flux<Optional<JsonNode>> div(BigDecimal leftNumber, BigDecimal rightNumber) {
	...
	public void evaluatePlusOnStrings() {
		...
		plus.evaluate(ctx, true, null).take(1).subscribe(result -> assertEquals(
	...
	public void evaluateElementOfOnWrongType() {
		...
		elementOf.evaluate(ctx, true, null).take(1).subscribe(result -> assertEquals(
	...
	public void evaluateRegExLeftUndefined() {
		...
		regex.evaluate(ctx, true, null).take(1).subscribe(result -> assertFalse(
	...
	public void evaluateRegExLeftWrongType() {
		...
		regEx.evaluate(ctx, true, null).take(1)
				.subscribe(result -> assertFalse("Null numver should evaluate to false",
						result.get().asBoolean()));
	}
