 ...
 */
package io.sapl.spring.data.database;
...
public interface R2dbcPersonRepository extends R2dbcRepository<Person, String> {
    ...
    @QueryEnforce(subject = "#firstname", action = "protection", resource = "#setResource('field', #firstname)", environment = "@r2dbcTestService.setEnvironment(#age, 2)", staticClasses = {
            TestClass.class })
    Flux<Person> findAllByAgeAfterAndFirstname(int age, String firstname);

    @QueryEnforce(subject = "#setResource('field', #firstname)")
    Flux<Person> findAllByAgeAfterAndId(int age, int id);

    Flux<Person> findAllByAgeAfter(int age);

    @QueryEnforce(subject = "#firstname", action = "protection", resource = "T(io.sapl.spring.data.database.TestClass).setResource(#firstname, 'test value')", environment = "{\"testNode\":\"testValue\"}", staticClasses = {})
    ...
    @QueryEnforce(subject = "test", action = "test", resource = "setResource", environment = "test", staticClasses = {
            TestClass.class })
    Flux<Person> findByIdAfterAndFirstname(int id, String firstname);

    @QueryEnforce(subject = "#setResource('field', #firstname)", action = "asdst", resource = "setResource", environment = "test", staticClasses = {})
