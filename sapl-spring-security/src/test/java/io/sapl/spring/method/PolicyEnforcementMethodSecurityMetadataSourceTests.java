...
class PolicyEnforcementMethodSecurityMetadataSourceTests {

	private PolicyEnforcementAttributeFactory attributeFactory;
	...
	void whenEver_ThenGetAllConfigAttributesIsNull() {
		var sut = new PolicyEnforcementMethodSecurityMetadataSource(attributeFactory);
	...
	void whenAnnotationOnClassOnly_ThenReturnsAnnotationFromClass() throws NoSuchMethodException, SecurityException {
		...
		assertThat((PolicyBasedPreInvocationEnforcementAttribute) attributes.get(0),
				is(pojo(PolicyBasedPreInvocationEnforcementAttribute.class).where("getSubjectExpression",
	...
	void whenAnnotationOnMethod_ThenReturnsOnMetodForPost() throws NoSuchMethodException, SecurityException {
		...
		var method = TestClass.class.getMethod("doSomething");
		List<ConfigAttribute> attributes = new ArrayList<>(sut.getAttributes(method, TestClass.class));
		assertThat((PolicyBasedPostInvocationEnforcementAttribute) attributes.get(0),
				is(pojo(PolicyBasedPostInvocationEnforcementAttribute.class).where("getSubjectExpression",
						is(pojo(Expression.class).where("getExpressionString", is("'onMethod'"))))));
	}
