...
/**
 * The streaming PEP's Mealy machine. Defines the alphabets and the combined
 * transition + output function
 * {@code S × Σ → S × Λ}, which is the defining property of a Mealy machine
 * (output depends on {@code (state, event)},
 * as opposed to Moore where output depends only on state). Four sealed types
 * form one
 ...
 * downstream).</li>
 * <li>{@link TransitionReason}. The discriminator for boundary signals surfaced
 * via
 * {@link Emission.EmitTransition}.</li>
 * </ul>
 * Plus the {@link Step} record packing a step's results, and the pure
 * {@link #step(State, Event)} function that
 * advances the machine.
 * <p>
 * Routing dispatches on the PDP decision verb (carried by the event) and the
 * current state. Explicit {@link PdpDeny}
 * always terminates; {@link PdpSuspend} always transitions to
 * {@link Suspended}. Under the strict fail-closed default,
 * a per-item obligation failure also terminates the subscription with
 * {@link AccessDeniedException}.
 * <p>
 * Boundary signals fire symmetrically on every entry into {@link Permitting}
 * from a non-Permitting state (initial grant
 * or resume) and on every entry into {@link Suspended} from a non-Suspended
 * state. Plan replacement
 * (Permitting&nbsp;-&gt;&nbsp;Permitting) and re-suspension
 * (Suspended&nbsp;-&gt;&nbsp;Suspended) are silent.
 *
 * @since 4.1.0
 */
@UtilityClass
public class MealyMachine {
    ...
    /**
     * Message text for {@link AccessDeniedException} when a per-item obligation
     * handler fails to enforce an item under
     * an otherwise-valid PERMIT. The subscription terminates.
     */
    public static final String DENIED_BY_OBLIGATION_FAILURE  = "Access denied: a per-item obligation handler failed.";
    /**
     * Message text for {@link AccessDeniedException} when access is denied by an
     * explicit {@code Decision.DENY} from
     * the PDP.
     */
    public static final String DENIED_BY_POLICY              = "Access denied by policy.";
    /**
     * Message text for {@link AccessDeniedException} when access is denied because
     * the PDP returned
    ...
    /**
     * Message text for {@link AccessDeniedException} when access is denied because
     * a granted PERMIT could not be
     * enforced at the decision-scoped enforcement phase.
     */
    public static final String DENIED_PERMIT_NOT_ENFORCEABLE = "Access denied: decision-scoped enforcement of permit failed.";

    /**
     * The state set of the streaming PEP's Mealy machine. Sealed into four cases
     * that together describe the entire
     * lifecycle of one subscription. Routing is driven by the PDP decision verb,
     * not by an annotation flag.
     */
    public sealed interface State {
        ...
        /**
         * The current decision permits and the {@link EnforcementPlan} is usable.
         * Per-item enforcement runs against
         * {@code plan}; lifecycle signals fire against it as long as it remains
         * current.
         *
         * @param plan
         * the active plan for this decision
         */
         ...
         * {@link Emission.EmitTransition}
         * rather than being stored on the state.
         */
    ...
    /**
     * The input alphabet of the streaming PEP's Mealy machine. Sealed into eight
     * cases covering three input domains:
     * PDP-side decision events, RAP-side stream events, and downstream
     * subscriber-side lifecycle events.
     * <p>
     * The pipeline pre-classifies PDP decisions: it receives a raw
     * {@link AuthorizationDecision}, builds the
     * {@link EnforcementPlan} (always succeeds; substitutes are baked into the
     * plan), runs the plan's decision-scoped
     * handlers, and emits the appropriate event variant by decision verb.
     */
    public sealed interface Event {

        /**
         * The PDP returned PERMIT and decision-scoped enforcement succeeded. The state
        ...
        /**
         * The PDP returned an explicit {@code Decision.SUSPEND}. The state machine
         * transitions to
         ...
         * INDETERMINATE or NOT_APPLICABLE under
         * the strict fail-closed discipline, or returned PERMIT whose decision-scoped
         * enforcement could not be
         * honoured. The {@link DenyKind} discriminates the cause. The state machine
         * transitions to
         * {@link State.Terminated} and emits a terminal {@link Emission.EmitError}
         * carrying an
         * {@link AccessDeniedException}.
         */
        ...
        /**
         * The PDP's decision flux raised. Treated as terminal: emit the throwable
         * downstream and transition to
    ...
    /**
     * The output alphabet of the streaming PEP's Mealy machine. Sealed into four
     * cases that cover what the state
     * machine can ask the Reactor adapter to deliver downstream on a single
     * transition.
     * <p>
     * The {@code step} function returns a {@link Step} carrying a
     * {@code List<Emission>} (the multi-output of the
     * MealyMachine formulation): a single step can produce zero, one, or several
     * emissions in order. Zero emissions
     * encode "this event was processed and produced nothing observable" (e.g. a
     * suspended-state item drop, or a mapper
     * that returned null).
     */
    public sealed interface Emission {

        /**
         * Deliver {@code value} to the subscriber. Used for permitted item pass-through
         * after per-item enforcement
         * succeeded.
         */
        record Emit(Object value) implements Emission {}

        /**
         * Terminate the subscriber with an error. Used for any denial (DENY,
         * INDETERMINATE, NOT_APPLICABLE, PERMIT with
         * failed decision-scoped enforcement), per-item obligation failure, RAP errors,
         * and PDP stream errors.
         */
        record EmitError(Throwable throwable) implements Emission {}

        /**
         * Terminate the subscriber normally. Used when the protected method (RAP)
        ...
        /**
         * Deliver an out-of-band transition signal carrying a {@link TransitionReason}.
         * Emitted on every suspend /
         * resume boundary crossing. The pipeline gates visibility on the annotation's
         * {@code signalTransitions} flag
         * and surfaces the signal as a non-terminal exception on the error channel
         * ({@link org.springframework.security.access.AccessDeniedException} for
         * suspension boundaries,
         * {@link AccessGrantedException} for resume), consumed via
         * {@code onErrorContinue} or
     ...
     * {@link Emission.EmitTransition} so subscribers (when the PEP
     * is configured with {@code signalTransitions = true}) can react to the
     * boundary, and used internally to format the
     * {@link org.springframework.security.access.AccessDeniedException} message on
     * terminal denial / item-enforcement
     * failure.
     */
    public sealed interface TransitionReason {

        /**
         * The subscription has been suspended by an explicit {@code Decision.SUSPEND}
         * from the PDP.
         *
         * @param decision
         * the decision that caused the suspension
         */
         ...
         * pending-to-permitting transition (initial grant)
         * and the suspended-to-permitting transition (resume). Plan replacement
    ...
    /**
     * Discriminator for {@link Event.PdpDeny}. The four causes are isomorphic from
     * the FSM's perspective (each
     * terminates the subscription with an {@link AccessDeniedException}); the kind
     * names which decision verb or which
     * enforcement outcome triggered the terminal denial, enabling distinct
     * exception messages and audit diagnostics.
     */
    ...
    /**
     * The codomain of the machine's combined step function. Pairs the post-step
     * {@link State} with the ordered list of
     * {@link Emission}s produced by the step (the multi-output of the MealyMachine
     * formulation).
     *
     * @param newState
     * the post-step state
     * @param emissions
     * the (possibly empty) emission sequence
     */
    public record Step(State newState, List<Emission> emissions) {

        /**
         * A step into {@code newState} producing the given emissions in order. Zero
         * emissions ({@code Step.to(state)})
         * means "event processed; nothing to emit downstream."
         */
        ...
        /**
         * @return {@code true} when {@link #newState()} is {@link State.Terminated};
         * the reactor adapter stops
    ...
    /**
     * Compute the next state and emissions for a single (state, event) pair. Pure:
     * no side effects, no Reactor types,
     * no Spring types.
     */
