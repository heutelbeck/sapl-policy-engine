...
/**
 * This Constraint Handler Provider can be used to apply validations on a field based on SAPL Obligations.
 * Use the method "{addConsumerConstraintHandlerProviders()}" on your builder to register a new instance of this class.
 * Fields can be registered with the method {@link #bindField(HasValue)}.
 *
 * This provider manages constrains of type "saplVaadin" with id "validation", here an example:
 * ...
 * obligation
 *     {
 *         "type": "saplVaadin",
 ...
 * ...
 *
 * Each field defined in the constraint "fields" dict should be bound using {@link #bindField(HasValue)}. We use a
 * slightly modified version of the JsonSchema define validations in the policy. Each field entry is transformed to
 * a single "properties" entry following the same syntax.
 * The additional "$schema" keyword can be used to define the json schema version for this validation.
 * The additional "message" keyword can be used to define a custom validator error message.
 */
public class FieldValidationConstraintHandlerProvider implements ConsumerConstraintHandlerProvider<UI> {
	...
	public FieldValidationConstraintHandlerProvider(Binder<?> binder, Object objectWithMemberFields, ObjectMapper objectMapper) {
		this.binder = binder;
		this.objectWithMemberFields = objectWithMemberFields;
	...
	/**
	 * This method is used by {@link VaadinConstraintEnforcementService} to get a handler
	 * for each constraint in the decision.
	 * The handler is a consumer functional interface consuming the ui related to the component specified in the builder.
	...
	/**
	 * Update the internal structures {@link #jsonValidationSchemaFromFieldName} and {@link #validationMessageFromFieldName}
	 * that are used by the validator if field is bound (see {@link #isBoundFromFieldName})
	...
	public <FIELDVALUE> FieldValidationConstraintHandlerProvider bindField(HasValue<?, FIELDVALUE> field){
		getFieldsInDeclareOrder(objectWithMemberFields.getClass())
				...
				.filter( memberField ->
					memberField.getType().isAssignableFrom(field.getClass())
				)
				.filter( memberField ->
					isFieldBound(memberField, field, objectWithMemberFields))
				.forEach(memberField -> {
		...
		return (value, context) -> {
			...
			if ( validationSchema != null ) {
				...
				if (value instanceof LocalTime) {
					convertedValue = objectMapper.convertValue(((LocalTime) value).format(DateTimeFormatter.ISO_TIME), JsonNode.class);
				} else if (value instanceof LocalDateTime){
					convertedValue = objectMapper.convertValue(((LocalDateTime) value).format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), JsonNode.class);
				}
	 ...
	 */
	<FIELDVALUE> boolean isFieldBound(Field memberField,
	 ...
	 * elements in the array returned are sorted in declare order from subclass
	 * to super class.
	 *
	 * @param searchClass
	 *            class to introspect
