...
public class StreamAttributeMethodSignatureProcessor {
    ...
    /**
     * Processes a method to create a {@link StreamAttributeFinderSpecification}.
     *
     * @param pipInstance the PIP instance to bind instance methods to,
     * or null for static methods
     * @param namespace the namespace prefix for the attribute
     * @param method the method annotated with {@link Attribute} or
     * {@link EnvironmentAttribute}
     * @return a specification for the method, or null if the method is
     * not annotated as an attribute
     * @throws IllegalStateException if the method is invalid (non-static
     * without instance, bad parameter or return type)
     */
    public static StreamAttributeFinderSpecification processAttributeMethod(Object pipInstance, String namespace,
            Method method) {
        ...
        val returnsStream = validateAndClassifyReturnType(method);

        val name          = extractAttributeName(method, hasAttribute);
        val signatureInfo = extractSignature(method, hasEnvironmentAttribute);

        try {
            val attributeFinder = createAttributeFinderForMethod(pipInstance, method, signatureInfo, returnsStream);
            return new StreamAttributeFinderSpecification(namespace, name, hasEnvironmentAttribute,
    ...
    /**
     * Validates the return type is either {@code Stream<Value>} (or a
     * subtype of {@link Value} as the type argument) or a raw
     * {@link Value} subtype. Returns {@code true} when the method
     * returns a Stream, {@code false} when it returns a bare Value.
     */
    private static boolean validateAndClassifyReturnType(Method method) {
        val returnType = method.getGenericReturnType();

        if (returnType instanceof Class<?> clazz && Value.class.isAssignableFrom(clazz)) {
            return false;
        }

        if (returnType instanceof ParameterizedType paramType && Stream.class.equals(paramType.getRawType())) {
            val typeArgs = paramType.getActualTypeArguments();
            if (typeArgs.length == 1 && isAssignableToValue(typeArgs[0])) {
                return true;
            }
        }
    ...
    private static boolean isAssignableToValue(Type type) {
    ...
    static StreamAttributeFinder createAttributeFinderForMethod(Object pipInstance, Method method,
            SignatureInfo signatureInfo, boolean returnsStream) throws IllegalAccessException {
    ...
    @SuppressWarnings("unchecked")
    private static Stream<Value> convertResultToStream(Object result, boolean returnsStream) {
        if (returnsStream) {
            return (Stream<Value>) result;
        }
        return Streams.just((Value) result);
    }
