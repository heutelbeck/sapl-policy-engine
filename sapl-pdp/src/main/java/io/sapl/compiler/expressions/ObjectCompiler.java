...
public class ObjectCompiler {
    ...
    public static CompiledExpression compile(ObjectExpression expr, CompilationContext ctx) {
        ...
        return buildFromCompiled(keys, compiled, expr.location(), ctx.errorShortCircuit());
    }
    ...
    static CompiledExpression buildFromCompiled(List<String> keys, List<CompiledExpression> compiled,
            SourceLocation location, boolean errorShortCircuit) {
        return buildFromCompiled(keys.toArray(String[]::new), compiled, location, errorShortCircuit);
    }
    ...
    private static CompiledExpression buildFromCompiled(String[] keys, List<CompiledExpression> compiled,
        ...
        if (cat.hasSingleStream()) {
            if (errorShortCircuit) {
                return new SingleStreamObjectLazy(keys, cat.valueIndices(), cat.values(), cat.pureIndices(),
                        cat.pureOperators(), cat.streamIndices()[0], cat.streams()[0], cat.totalCount(), compiled);
            }
            return new SingleStreamObjectEager(keys, cat.valueIndices(), cat.values(), cat.pureIndices(),
                    cat.pureOperators(), cat.streamIndices()[0], cat.streams()[0], cat.totalCount(), compiled);
        }
        if (errorShortCircuit) {
            return new MultiStreamObjectLazy(keys, cat.valueIndices(), cat.values(), cat.pureIndices(),
                    cat.pureOperators(), cat.streamIndices(), cat.streams(), cat.totalCount(), compiled);
        }
        return new MultiStreamObjectEager(keys, cat.valueIndices(), cat.values(), cat.pureIndices(),
                cat.pureOperators(), cat.streamIndices(), cat.streams(), cat.totalCount(), compiled);
    }
    ...
    /**
     * Object with exactly one stream value. Lazy variant: snapshot
    ...
    record SingleStreamObjectLazy(
            ...
            int totalEntries,
            List<CompiledExpression> compiledValues) implements StreamOperator {
        ...
        public ExpressionResult evaluate(EvaluationContext ctx) {
            return assembleObjectLazy(keys, compiledValues, ctx);
        }
        ...
        public int hashCode() {
            return Objects.hash(Arrays.hashCode(keys), Arrays.hashCode(valueIndices), Arrays.hashCode(values),
                    ...
                    Objects.hashCode(streamOp), totalEntries, Objects.hashCode(compiledValues));
        }

        @Override
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof SingleStreamObjectLazy(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreamOp, var oTotal, var oCompiled)
                            ...
                            && Objects.equals(compiledValues, oCompiled));
        }
    }

    /**
     * Object with exactly one stream value. Eager variant: snapshot
    ...
    record SingleStreamObjectEager(
        ...
        @Override
        public ExpressionResult evaluate(EvaluationContext ctx) {
            return assembleObjectEager(keys, compiledValues, ctx);
        }
        ...
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof SingleStreamObjectEager(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreamOp, var oTotal, var oCompiled)
                            && Arrays.equals(keys, oKeys) && Arrays.equals(valueIndices, oValIdx)
                            && Arrays.equals(values, oVals) && Arrays.equals(pureIndices, oPureIdx)
                            && Arrays.equals(pureOperators, oPureOps) && streamIndex == oStreamIdx
                            && Objects.equals(streamOp, oStreamOp) && totalEntries == oTotal
    ...
    /**
     * Object with multiple stream values. Lazy variant: snapshot
     * {@code evaluate(ctx)} short-circuits on the first {@link ErrorValue}
     * encountered. Selected at compile time when the
     * {@code errorShortCircuit} compiler option is enabled.
     */
    record MultiStreamObjectLazy(
            String[] keys,
            int[] valueIndices,
            Value[] values,
            int[] pureIndices,
        ...
        public int hashCode() {
            return Objects.hash(Arrays.hashCode(keys), Arrays.hashCode(valueIndices), Arrays.hashCode(values),
                    ...
                    Arrays.hashCode(streams), totalEntries, Objects.hashCode(compiledValues));
        }

        @Override
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof MultiStreamObjectLazy(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreams, var oTotal, var oCompiled)
                            && Arrays.equals(keys, oKeys) && Arrays.equals(valueIndices, oValIdx)
                            && Arrays.equals(values, oVals) && Arrays.equals(pureIndices, oPureIdx)
    ...
    /**
     * Object with multiple stream values. Eager variant: snapshot
     * {@code evaluate(ctx)} walks every child to accumulate the maximum
     * subscription set, holds the first {@link ErrorValue}, and returns it
     * after the full walk. Selected at compile time when the
     * {@code errorShortCircuit} compiler option is disabled (default).
    ...
    record MultiStreamObjectEager(
        ...
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof MultiStreamObjectEager(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreams, var oTotal, var oCompiled)
                            && Arrays.equals(keys, oKeys) && Arrays.equals(valueIndices, oValIdx)
                            && Arrays.equals(values, oVals) && Arrays.equals(pureIndices, oPureIdx)
                            && Arrays.equals(pureOperators, oPureOps) && Arrays.equals(streamIndices, oStreamIdx)
                            && Arrays.equals(streams, oStreams) && totalEntries == oTotal
    ...
    /**
     * Lazy assembly: walks compiled values left-to-right via
     ...
     * entries whose value is {@link UndefinedValue} per object literal
     * semantics. {@code null} from a child sets the incomplete flag but
     * iteration continues to maximize the subscription set.
     */
    private static ExpressionResult assembleObjectLazy(String[] keys, List<CompiledExpression> compiledValues,
        ...
        for (int i = 0; i < compiledValues.size(); i++) {
            val v = evalChild(compiledValues.get(i), ctx, subs);
            if (v == null) {
                seenNull = true;
                continue;
            }
    ...
    /**
     * Eager assembly: walks every compiled value via
     ...
     * after the full walk completes. Drops entries whose value is
     * {@link UndefinedValue} per object literal semantics. {@code null} from a
     * child sets the incomplete flag; on a clean walk with no error, returns
     * the assembled object. Precedence at the end:
