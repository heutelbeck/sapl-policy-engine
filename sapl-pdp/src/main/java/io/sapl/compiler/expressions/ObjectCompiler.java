...
public class ObjectCompiler {
    ...
    /**
     * Object with exactly one stream value. {@code evaluate(ctx)} walks
    ...
    record SingleStreamObject(
        ...
        public ExpressionResult evaluate(EvaluationContext ctx) {
            return assembleObject(keys, compiledValues, ctx);
        }
        ...
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof SingleStreamObject(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreamOp, var oTotal, var oCompiled)
    ...
    /**
     * Object with multiple stream values. {@code evaluate(ctx)} walks every
     * child to accumulate the maximum subscription set, holds the first
    ...
    record MultiStreamObject(
        ...
        public boolean equals(Object o) {
            return this == o
                    || (o instanceof MultiStreamObject(var oKeys, var oValIdx, var oVals, var oPureIdx, var oPureOps, var oStreamIdx, var oStreams, var oTotal, var oCompiled)
                            && Arrays.equals(keys, oKeys) && Arrays.equals(valueIndices, oValIdx)
                            && Arrays.equals(values, oVals) && Arrays.equals(pureIndices, oPureIdx)
    ...
    /**
     * Walks every compiled value via {@link StreamOperator#evalChild},
     * accumulating subscriptions even past any encountered
     * {@link ErrorValue}. Holds the first error and returns it after the
     * full walk completes. Drops entries whose value is
     * {@link UndefinedValue} per object literal semantics. {@code null}
     * from a child sets the incomplete flag; on a clean walk with no error,
    ...
    private static ExpressionResult assembleObject(String[] keys, List<CompiledExpression> compiledValues,
        ...
        for (int i = 0; i < compiledValues.size(); i++) {
            val v = evalChild(compiledValues.get(i), ctx, deps);
            if (v == null) {
                seenNull = true;
                continue;
            }
