...
public class ExpressionCompiler {
    ...
    public CompiledExpression compileExpression(Expression expression, CompilationContext context) {
        ...
        case EagerOr eagerOr       ->
            compileBinaryOperator(eagerOr, eagerOr.getLeft(), eagerOr.getRight(), BooleanOperators::or, context);
        case EagerAnd eagerAnd     ->
            compileBinaryOperator(eagerAnd, eagerAnd.getLeft(), eagerAnd.getRight(), BooleanOperators::and, context);
        case XOr xor               ->
            compileBinaryOperator(xor, xor.getLeft(), xor.getRight(), BooleanOperators::xor, context);
        case Not not               -> compileUnaryOperator(not, not.getExpression(), BooleanOperators::not, context);
        case Multi multi           ->
            compileBinaryOperator(multi, multi.getLeft(), multi.getRight(), NumberOperators::multiply, context);
        case Div div               ->
            compileBinaryOperator(div, div.getLeft(), div.getRight(), NumberOperators::divide, context);
        case Modulo modulo         ->
            compileBinaryOperator(modulo, modulo.getLeft(), modulo.getRight(), NumberOperators::modulo, context);
        case Plus plus             ->
            compileBinaryOperator(plus, plus.getLeft(), plus.getRight(), NumberOperators::add, context);
        case Minus minus           ->
            compileBinaryOperator(minus, minus.getLeft(), minus.getRight(), NumberOperators::subtract, context);
        case Less less             ->
            compileBinaryOperator(less, less.getLeft(), less.getRight(), NumberOperators::lessThan, context);
        case LessEquals lessEquals -> compileBinaryOperator(lessEquals, lessEquals.getLeft(), lessEquals.getRight(),
                NumberOperators::lessThanOrEqual, context);
        case More more             ->
            compileBinaryOperator(more, more.getLeft(), more.getRight(), NumberOperators::greaterThan, context);
        case MoreEquals moreEquals -> compileBinaryOperator(moreEquals, moreEquals.getLeft(), moreEquals.getRight(),
                NumberOperators::greaterThanOrEqual, context);
        case UnaryPlus unaryPlus   ->
            compileUnaryOperator(unaryPlus, unaryPlus.getExpression(), NumberOperators::unaryPlus, context);
        case UnaryMinus unaryMinus ->
            compileUnaryOperator(unaryMinus, unaryMinus.getExpression(), NumberOperators::unaryMinus, context);
        case ElementOf elementOf   -> compileBinaryOperator(elementOf, elementOf.getLeft(), elementOf.getRight(),
                ComparisonOperators::isContainedIn, context);
        case Equals equals         ->
            compileBinaryOperator(equals, equals.getLeft(), equals.getRight(), ComparisonOperators::equals, context);
        case NotEquals notEquals   -> compileBinaryOperator(notEquals, notEquals.getLeft(), notEquals.getRight(),
                ComparisonOperators::notEquals, context);
        case Regex regex           -> compileBinaryOperator(regex, regex.getLeft(), regex.getRight(),
                ComparisonOperators::matchesRegularExpression, context);
     ...
     * the left operand expression from the AST
     * @param rightExpression
     * the right operand expression from the AST
     * @param operator
     ...
     */
    private CompiledExpression compileBinaryOperator(EObject astNode, Expression leftExpression,
            Expression rightExpression, BiOperator operator, CompilationContext context) {

        val left  = compileExpression(leftExpression, context);
        val right = compileExpression(rightExpression, context);
        if (left == null) {
            return Error.at(astNode, ValueMetadata.EMPTY, RUNTIME_ERROR_LEFT_OPERAND_MISSING,
                    astNode.getClass().getSimpleName());
        }
        if (right == null) {
            return Error.at(astNode, ValueMetadata.EMPTY, RUNTIME_ERROR_RIGHT_OPERAND_MISSING,
                    astNode.getClass().getSimpleName());
        }
        ...
        // 99% of the logic.
        if (right instanceof Value valRight && astNode instanceof Regex) {
            val compiledRegex = ComparisonOperators.compileRegularExpressionOperator(astNode, valRight);
            operator = (node, l, ignoredBecauseWeUseTheCompiledRegex) -> compiledRegex.apply(l);
        }
        return assembleBinaryOperation(astNode, left, right, operator);
    }
     ...
     * the operand expression from the AST
     * @param operation
     ...
     */
    private CompiledExpression compileUnaryOperator(EObject astNode, Expression operandExpression,
            BiFunction<EObject, Value, Value> operation, CompilationContext context) {
        val compiled = compileExpression(operandExpression, context);
        return switch (compiled) {
        case Value value                          -> operation.apply(astNode, value);
        case PureExpression pureExpression        -> new PureExpression(
                ctx -> operation.apply(astNode, pureExpression.evaluate(ctx)), pureExpression.isSubscriptionScoped());
        case StreamExpression(Flux<Value> stream) -> new StreamExpression(stream.map(v -> operation.apply(astNode, v)));
        };
    }
     ...
     * @return the compiled step expression
     */
    private CompiledExpression compileStep(CompiledExpression parent, UnaryOperator<Value> operation) {
