...
class FilterArgumentEvaluator {
     ...
     */
    static List<Value> evaluatePureArguments(ParserRuleContext astNode, CompiledArguments arguments, Value leftHandArg,
     ...
     */
    static Flux<List<Value>> combineStreamArguments(CompiledArguments arguments, Value leftHandArg) {
        val sources = Arrays.stream(arguments.arguments()).map(ExpressionCompiler::compiledExpressionToFlux).toList();

        return Flux.combineLatest(sources, argValues -> {
