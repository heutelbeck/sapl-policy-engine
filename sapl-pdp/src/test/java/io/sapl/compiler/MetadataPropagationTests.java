...
class MetadataPropagationTests {
    ...
    void setUp() {
        ...
        evaluationContext = new EvaluationContext("testConfigurationId", "testSubscriptionId", subscription,
    ...
    void when_attributeFinderInvoked_then_resultContainsAttributeRecord() {
        val result = evaluate("<meta.cultist>");
    ...
    void when_entityIsSecret_then_resultIsSecretWithTrace() {
        ...
        val result       = evaluateWithVariable("entity.<meta.echo>", "entity", secretEntity);
    ...
    void when_chainedAttributeSteps_then_tracesAccumulateWithCorrectNames() {
        val result = evaluate("<meta.cultist>.<meta.echo>");
    ...
    void when_functionWithAttributeArgument_then_resultHasTrace() {
        val result = evaluate("meta.identity(<meta.cultist>)");
    ...
    void when_functionWithMultipleAttributeArguments_then_allTracesMerged() {
        val result = evaluate("meta.combine(<meta.cultist>, <meta.ritual>)");
    ...
    void when_deeplyNestedFunctions_then_attributeTracePreserved(int depth) {
        val expression = "meta.identity(".repeat(depth) + "<meta.cultist>" + ")".repeat(depth);
    ...
    void when_nestedFunctionsWithDifferentAttributes_then_allTracesAccumulated() {
        val result = evaluate("meta.combine(meta.identity(<meta.cultist>), meta.identity(<meta.ritual>))");
    ...
    @CsvSource({ "equality,       '<meta.cultist> == <meta.ritual>',     2",
            "inequality,     '<meta.cultist> != <meta.ritual>',     2",
            "addition,       '<meta.number> + <meta.number>',       2",
            "subtraction,    '<meta.number> - <meta.number>',       2",
            "multiplication, '<meta.number> * <meta.number>',       2",
            "boolean AND,    '<meta.truth> && <meta.truth>',        2",
            "boolean OR,     '<meta.falsehood> || <meta.truth>',    2",
            "less than,      '<meta.number> < <meta.number>',       2",
            "greater than,   '<meta.number> > <meta.number>',       2" })
    ...
    void when_booleanOrShortCircuits_then_onlyEvaluatedOperandMetadataPreserved() {
        val result = evaluate("<meta.truth> || <meta.cultist>");
    ...
    @CsvSource({ "boolean NOT,  '!<meta.truth>'", "unary minus,  '-<meta.number>'" })
    ...
    @CsvSource({ "array,  '[<meta.cultist>, <meta.ritual>]',                          2",
            "object, '{ \"a\": <meta.cultist>, \"b\": <meta.ritual> }',          2" })
    ...
    @CsvSource({ "key,              '<meta.tome>.title',         1", "index,            '<meta.artifacts>[0]',       1",
            "recursive key,    '<meta.tome>..title',        1", "wildcard,         '<meta.tome>.*',             1",
            "array slicing,    '<meta.artifacts>[0:2]',     1" })
    ...
    @CsvSource({ "simple filter,                       '<meta.cultist> |- meta.identity',                         1",
            "each filter with attribute elements,  '[<meta.cultist>, <meta.ritual>] |- each meta.identity',   2" })
    ...
    void when_conditionStepWithAttributeComparison_then_tracesAccumulated() {
        val result = evaluate("<meta.artifacts>[?(@ == \"Silver Key\")]");
    ...
    void when_attributeReturnsError_then_errorHasAttributeTrace() {
        val result = evaluate("<meta.forbidden>");
    ...
    static Stream<Arguments> complexExpressionTestCases() {
        return Stream.of(
                arguments("function result as attribute entity", "meta.identity(<meta.cultist>).<meta.echo>", 2),
                arguments("nested function with step", "meta.identity(meta.identity(<meta.tome>).title)", 1),
                arguments("attributes in comparison",
                        "meta.combine(<meta.cultist>, <meta.ritual>) == meta.identity(<meta.tome>.title)", 3),
                arguments("attributes in container in function", "meta.identity([<meta.cultist>, <meta.ritual>])", 2),
                arguments("attributes combined and filtered", "[<meta.cultist>, <meta.ritual>] |- each meta.identity",
                        2));
    }
    ...
    private Value evaluate(String expression) {
        ...
        val compiledExpression = ExpressionCompiler.compileExpression(parsedExpression, compilationContext);
