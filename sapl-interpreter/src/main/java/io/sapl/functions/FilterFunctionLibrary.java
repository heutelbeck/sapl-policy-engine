...
public class FilterFunctionLibrary {
	...
	private static final String BLACKEN_DOC = "blacken(STRING, SHOW_LEFT, SHOW_RIGHT, REPLACEMENT): Assumes that SHOW_LEFT and SHOW_RIGHT are positive integers and STRING and REPLACEMENT are strings."
			+ " Replaces each character in STRING by REPLACEMENT, leaving SHOW_LEFT chars from the beginning and SHOW_RIGHT chars from the end unchanged."
			+ " Except for STRING, all parameters are optional."
			+ " SHOW_LEFT defaults to 0, SHOW_RIGHT defaults to 0 and REPLACEMENT defaults to 'X'"
			+ " Returns the modified STRING.";
	private static final String REPLACE_DOC = "replace(ORIGINAL, REPLACEMENT): Assumes that ORIGINAL and REPLACEMENT are JSON nodes. Returns VALUE.";

	private static final String ILLEGAL_PARAMETERS_COUNT = "Illegal number of parameters provided.";
	private static final String ILLEGAL_PARAMETER_SHOW_LEFT = "Illegal parameter for SHOW_LEFT. Expecting a positive integer.";
	private static final String ILLEGAL_PARAMETER_SHOW_RIGHT = "Illegal parameter for SHOW_RIGHT. Expecting a positive integer.";
	...
	public static JsonNode blacken(JsonNode... parameters) throws FunctionException {
		...
		if (parameters.length >= THREE) {
			if (!parameters[TWO].isNumber() || parameters[TWO].asInt() < 0)
				throw new FunctionException(ILLEGAL_PARAMETER_SHOW_RIGHT);
			showRight = parameters[TWO].asInt();
		} else {
			showRight = BLACKEN_DEFAULT_SHOW_RIGHT;
		}

		int showLeft;
		if (parameters.length >= TWO) {
			if (!parameters[ONE].isNumber() || parameters[ONE].asInt() < 0)
				throw new FunctionException(ILLEGAL_PARAMETER_SHOW_LEFT);
			showLeft = parameters[ONE].asInt();
		} else {
			showLeft = BLACKEN_DEFAULT_SHOW_LEFT;
		}

		String string = parameters[ZERO].asText();
		StringBuilder result = new StringBuilder();
		if (showLeft + showRight < string.length()) {
			if (showLeft > 0) {
				result.append(string.substring(0, showLeft));
			}

			int replacedChars = string.length() - showLeft - showRight;
			for (int i = 0; i < replacedChars; i++)
				result.append(replacement);

			if (showRight > 0) {
				result.append(string.substring(showLeft + replacedChars));
			}
		} else {
