...
public class FastIndexCreationStrategy implements IndexCreationStrategy {
	...
	@Override
	public IndexContainer construct(final Map<String, SAPL> documents,
			final Map<String, DisjunctiveFormula> targets) {
		Map<String, SAPL> idToDocument = ImmutableMap.copyOf(documents);

		Map<DisjunctiveFormula, Set<SAPL>> formulaToDocuments = mapFormulaToDocuments(
				targets, idToDocument);
		...
		Map<Integer, Set<DisjunctiveFormula>> indexToTargets = mapIndexToTargets(
				clauseToIndexBiMap, clauseToFormulas);

		Map<DisjunctiveFormula, Bitmask> formulaToClauses = mapFormulaToClauses(
				formulaToDocuments.keySet(), clauseToIndexBiMap);

		int[] indexToLengthTemplate = mapIndexToLength(clauseToIndexBiMap.inverse());
		int[] indexToOccurencesTemplate = mapIndexToOccurences(
				clauseToIndexBiMap.inverse(), clauseToFormulas);
		AuxiliaryMatrix matrix = new AuxiliaryMatrix(indexToLengthTemplate,
				indexToOccurencesTemplate);

		return new FastIndexContainer(true, variableOrder, formulaToClauses,
				flattenIndexMap(indexToTargets), formulaToDocuments, matrix);
	}

	private static Collection<VariableInfo> collectData(
			...
			for (ConjunctiveClause clause : formula.getClauses()) {
				...
				for (Literal literal : literals) {
					collectDataImpl(literal, clause, boolToVariableInfo,
							negativesGroupedByFormula, positivesGroupedByFormula,
							sizeOfClause);
				}
			}
		}
		for (VariableInfo variableInfo : boolToVariableInfo.values()) {
			double sum = variableInfo.getClauseRelevancesList().stream()
					.mapToDouble(x -> x).sum();
			sum /= (variableInfo.getNumberOfPositives()
					+ variableInfo.getNumberOfNegatives());
			variableInfo.setRelevance(sum);
		}
		return boolToVariableInfo.values();
	}

	private static void collectDataImpl(final Literal literal,
			final ConjunctiveClause clause,
			final Map<Bool, VariableInfo> boolToVariableInfo,
			final Set<Bool> negativesGroupedByFormula,
	...
	private static BiMap<ConjunctiveClause, Integer> createCandidateOrder(
			final Collection<VariableInfo> data) {
		...
		for (VariableInfo variableInfo : data) {
			...
			for (ConjunctiveClause clause : variableInfo
					.getSetOfUnsatisfiableClausesIfTrue()) {
			...
			for (ConjunctiveClause clause : variableInfo
					.getSetOfUnsatisfiableClausesIfFalse()) {
				Integer index = result.get(clause);
				if (index == null) {
					index = i;
					result.put(clause, index);
	...
	private static double createScore(final VariableInfo variableInfo) {
		...
		return ((Math.pow(relevance, square - relevance)
				* (groupedPositives + groupedNegatives)) / costs)
				* (square - Math.pow((groupedPositives - groupedNegatives)
						/ (groupedPositives + groupedNegatives), square));
	}

	private static List<Variable> createVariableOrder(
			final Collection<VariableInfo> data) {
	...
	private static Map<DisjunctiveFormula, Bitmask> mapFormulaToClauses(
			final Collection<DisjunctiveFormula> formulas,
	...
	private static int[] mapIndexToLength(
			final Map<Integer, ConjunctiveClause> indexToClause) {
		final int[] result = new int[indexToClause.size()];
		indexToClause.forEach((key, value) -> result[key] = value.size());
		return result;
	}

	private static int[] mapIndexToOccurences(
			final Map<Integer, ConjunctiveClause> indexToClause,
			final Map<ConjunctiveClause, Set<DisjunctiveFormula>> clauseToFormulas) {
		final int[] result = new int[indexToClause.size()];
		indexToClause.forEach(
				(key, value) -> result[key] = clauseToFormulas.get(value).size());
		return result;
	}
	...
	private static Map<Integer, Set<DisjunctiveFormula>> mapIndexToTargets(
		...
		Map<Integer, Set<DisjunctiveFormula>> result = new HashMap<>(
				clauseToIndex.size());
		for (Map.Entry<ConjunctiveClause, Set<DisjunctiveFormula>> entry : clauseToFormulas
				.entrySet()) {
