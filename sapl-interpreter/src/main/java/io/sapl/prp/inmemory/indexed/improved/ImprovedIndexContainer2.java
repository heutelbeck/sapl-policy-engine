...
public class ImprovedIndexContainer2 implements IndexContainer {
...
//            orphanedCandidates = new Bitmask();
    ...
    private void removeCandidatesRelatedToPredicate(Predicate predicate, Bitmask candidates) {
    ...
    private Bitmask findOrphanedCandidates(Bitmask candidates, Bitmask satisfiableCandidates,
        ...
        satisfiableCandidates.forEachSetBit(index -> {
            ...
            for (CTuple cTuple : cTuples) {
                if (!candidates.isSet(cTuple.cI)) continue;
                eliminatedFormulasWithConjunction[cTuple.cI] += cTuple.n;

                // if all formular of conjunction have been eliminated
                if (eliminatedFormulasWithConjunction[cTuple.cI] == numberOfFormulasWithConjunction[cTuple.cI])
                    result.set(cTuple.cI);
            }
        });
    ...
    private Bitmask findSatisfiableCandidates(Bitmask candidates, Predicate predicate, boolean evaluationResult,
                                              int[] trueLiteralsOfConjunction) {
        Bitmask result = new Bitmask();
