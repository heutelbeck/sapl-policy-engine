...
public class DomainUtil {
	...
	public void writePolicyToFile(DomainPolicy policy, String policyPath) {
		String policyFileName = String.format("%s/%03d_%s.sapl", policyPath, DomainUtil.getNextPolicyCount(),
				policy.getFileName());
	...
	// return
	// roles.stream().map(DomainRole::getRoleName).collect(Collectors.toList());
	// }
	//
	// public static List<String> getResourceNames(List<DomainResource> resources) {
	// return
	// resources.stream().map(DomainResource::getResourceName).collect(Collectors.toList());
	// }
	//
	// public static String getResourcesStringForFileName(List<DomainResource>
	// resources) {
	...
	// }
	//
	// public static List<String> getExtendedRoleNames(List<ExtendedDomainRole>
	// roles) {
	// return
	// roles.stream().map(ExtendedDomainRole::getRole).map(DomainRole::getRoleName)
	// .collect(Collectors.toList());
	// }
	//
	// public static String getExtendedRoleIndicator(ExtendedDomainRole role) {
	// return String.format("b=%d,o=%d,a=%d,t=%d",
	// role.isBodyPresent() ? 1 : 0, role.isObligationPresent() ? 1 : 0,
	// role.isAdvicePresent() ? 1 : 0, role
