...
public class TestRunner {
	...
	private List<XlsRecord> run(PolicyGeneratorConfiguration config, String policyFolder,
		...
		try {
			...
			ImmutableParsedDocumentIndex initializedIndex = initializeIndex(benchmarkDataContainer.getIndexType(),
					policyFolder);
	...
	private void warmUp(PolicyGenerator generator, ImmutableParsedDocumentIndex documentIndex) {
		EvaluationContext emptySubscriptionScopedEvaluationCtx = new EvaluationContext(ATTRIBUTE_CONTEXT,
				FUNCTION_CONTEXT, VARIABLES).forAuthorizationSubscription(generator.createEmptySubscription());
		try {
			for (int i = 0; i < 10; i++) {
