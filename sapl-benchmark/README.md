...
This benchmark suite produces reproducible, traceable performance data for SAPL. Every published number traces to a stored CSV file. Every remote measurement is validated against Little's Law. Deviations from this methodology are documented per data point.
...
taskset -c 0-15 java -Xmx32g -jar sapl-node.jar benchmark --rbac -t 8
```
...
```bash
java -jar sapl-benchmark-sapl4.jar --scenario rbac --method decideOnceBlocking -t 8
java -jar sapl-benchmark-sapl4.jar --scenario rbac --method decideStreamFirst -t 8
```

### Publication benchmarks vs quick assessment

The `sapl-benchmark-sapl4` module is the rigorous benchmark tool. It uses `forks(1)` on a flat classpath, giving JMH full control over JVM isolation. All published numbers come from this tool.

The `sapl-node benchmark` command is a quick assessment tool for users to evaluate their own policy sets. It runs in the Spring Boot fat JAR with `forks(0)` (required by the nested JAR classloader). Numbers from this tool are not used in publications. The `forks(0)` vs `forks(1)` equivalence is validated and stored in `reference/fork-comparison/`.
...
Every number in a published document traces to a specific CSV file in this repository. Embedded results are stored as dated CSV and Markdown reports. HTTP and RSocket results are stored as CSV matrices with columns for cores, connections, throughput, latency percentiles, and temperature. OPA results use the same CSV format.

If a measurement is not stored in a file, it does not exist.

### Visualization

Charts and tables in published documents are generated by scripts from the stored CSV data. No hand-authored visualizations. The pipeline:
...
```bash
taskset -c 0-15 java -Xmx32g -jar sapl-node/target/sapl-node-4.0.0-SNAPSHOT.jar benchmark --rbac
taskset -c 0-15 java -Xmx32g -jar sapl-node/target/sapl-node-4.0.0-SNAPSHOT.jar benchmark --rbac -b decideStreamFirst -t 8
```
...
taskset -c 0-15 java -Xmx32g -jar sapl-benchmark/sapl-benchmark-sapl4/target/sapl-benchmark-sapl4-4.0.0-SNAPSHOT.jar --scenario rbac -t 8
```
