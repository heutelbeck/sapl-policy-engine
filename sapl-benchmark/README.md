...
For HTTP, the wrk2 generator runs one thread per pinned client core (`WRK_THREADS`, default = client core count, recorded in `log_env`; override by exporting `WRK_THREADS`). Two threads (an earlier default) cannot saturate a fast server and undercount throughput: the reported number must be the server's ceiling, not the generator's. RSocket load is generated by the bundled `loadtest` tool (no adequate off-the-shelf RSocket generator exists); its HTTP mode is intentionally not used for benchmarking, as it cannot saturate a fast server.
...
| Latency percentiles (p50, p90, p99, p99.9) | JMH SampleTime (embedded); loadtest coordinated-omission (RSocket) |
...
| `server-http.sh` | Scenarios x cores x connections (HTTP via wrk2, one thread per client core) |
...
scripts/
  ...
  lib/
    ...
    sapl-wrk.lua                 # wrk2 POST with subscription from file
    opa-wrk.lua                  # wrk2 POST for OPA
  ...
  run-server-http.sh             # HTTP server benchmark via wrk2
