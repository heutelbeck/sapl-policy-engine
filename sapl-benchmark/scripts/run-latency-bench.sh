...
case "$PROFILE" in
    ...
    *)
        ...
        echo "Available: quick, rigorous, hospital-scaling, hospital-index"
        exit 1
        ;;
esac
...
TOTAL_STEPS=$(( ${#APPS[@]} * ${#SCALING_FACTORS[@]} * ${#GC_SWEEP[@]} * ${#INDEXING_SWEEP[@]} * SEEDS ))
CURRENT_STEP=0
        ...
        for seed in $(seq 0 $((SEEDS - 1))); do
            for app in "${APPS[@]}"; do
                for n in "${SCALING_FACTORS[@]}"; do
                    ...
                    echo "  $scenario / seed=$seed / $indexing / $gc / CPUs $cpu_range"
                    echo "================================================================"
                    wait_cool
            ...
            echo "  Completed seed $seed ($indexing / $gc)"
