...
# JVM latency benchmark (JMH SampleTime): measures p50/p90/p99/p99.9 across
# scenarios and scaling factors with multiple random entity graphs per data point.
#
# Usage: run-latency-jvm.sh [quick|full] [experiment] [output-dir]
...
OUTPUT_DIR=${3:-$SCRIPT_DIR/../results/latency-jvm-${EXPERIMENT}-$(timestamp)}
...
OUTDIR="$OUTPUT_DIR/latency-jvm-${EXPERIMENT}-${QUALITY}-${RUN_TIMESTAMP}"
mkdir -p "$OUTDIR"
