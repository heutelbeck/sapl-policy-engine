...
public class ClasspathHelper {
	...
	public static Path findPathOnClasspath(@NonNull ClassLoader loader, @NonNull String path) {
		...
		if (loader instanceof URLClassLoader) {
			errorMessage.append(System.lineSeparator()).append(System.lineSeparator()).append("We tried the following paths:").append(System.lineSeparator());
			URL[] classpathElements = ((URLClassLoader) loader).getURLs();
			for (URL classpathElement : classpathElements) {
				errorMessage.append("    - ").append(classpathElement.toString());
			}
		}
