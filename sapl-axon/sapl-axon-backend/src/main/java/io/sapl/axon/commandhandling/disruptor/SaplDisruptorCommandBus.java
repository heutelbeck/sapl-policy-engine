 ...
 *
 */
public class SaplDisruptorCommandBus extends DisruptorCommandBus {

    private static final Logger logger = LoggerFactory.getLogger(SaplDisruptorCommandBus.class);
    ...
    private final SaplCommandHandlerInvoker[] commandHandlerInvokers;
     ...
     *
     * @return a Builder to be able to create a {@link SaplDisruptorCommandBus}
     */
    public static Builder builderSapl() {
        return new Builder();
    }

    /**
     * Instantiate a {@link SaplDisruptorCommandBus} based on the fields contained in the {@link Builder}. The Threads
     ...
     *
     * @param builder the {@link Builder} used to instantiate a {@link SaplDisruptorCommandBus} instance
     */
    @SuppressWarnings("deprecation") // we use our own thread pool for disruptor
    protected SaplDisruptorCommandBus(Builder builder) {
    	super(DisruptorCommandBus.builder());
        ...
        if (executor == null) {
            executorService = Executors.newCachedThreadPool(new AxonThreadFactory("SaplDisruptorCommandBus"));
        ...
        var publishers = initializePublisherThreads(builder.publisherThreadCount,
                                                                 executor,
        ...
        disruptor.setDefaultExceptionHandler(new DisruptorExceptionHandler(disruptor));
    ...
    private EventPublisher[] initializePublisherThreads(int publisherThreadCount,
        ...
        var publishers = new EventPublisher[publisherThreadCount];
        Arrays.setAll(publishers, t -> new EventPublisher(executor, transactionManager, rollbackConfiguration, t));
        return publishers;
    }

    private SaplCommandHandlerInvoker[] initializeInvokerThreads(int invokerThreadCount, Cache cache) {
        var invokers = new SaplCommandHandlerInvoker[invokerThreadCount];
        Arrays.setAll(invokers, t -> new SaplCommandHandlerInvoker(cache, t, pep));
        return invokers;
    }

    @Override
    @SuppressWarnings("unchecked") // SuppressWarning from axon framework
    ...
    private <C, R> void doDispatch(CommandMessage<? extends C> command, CommandCallback<? super C, R> callback) {
        ...
        var aggregateIdentifier = commandTargetResolver.resolveTarget(command).getIdentifier();
        ...
        try {
	        ...
	        var blackListCallback =  new BlacklistDetectingCallback<C, R>(callback, disruptor.getRingBuffer(), this::doDispatch,
	                rescheduleOnCorruptState);
			event.reset(command, commandHandler, invokerSegment, publisherSegment, blackListCallback,
    ...
    public <T> Repository<T> createRepository(EventStore eventStore, AggregateFactory<T> aggregateFactory,
        ...
        for (var invoker : commandHandlerInvokers) {
        ...
        return new SaplDisruptorRepository<>(aggregateFactory.getAggregateType());
    }
    ...
    public Registration subscribe(String commandName, MessageHandler<? super CommandMessage<?>> handler) {
        ...
        commandHandlers.compute(commandName, (cn, existingHandler) -> {
            MessageHandler<? super CommandMessage<?>> handlerLocal;
        	if (existingHandler == null || existingHandler == handler) {
        		handlerLocal = handler;
            } else {
            	handlerLocal = duplicateCommandHandlerResolver.resolve(commandName, existingHandler, handler);
            }
        	pep.gatherNecessaryHandlers(commandName, handlerLocal, commandTargetResolver);
        	return handlerLocal;
        });
    ...
    public void stop() {
        ...
        while (System.currentTimeMillis() - lastChangeDetected < coolingDownPeriod && !Thread.interrupted()) {
            // no test possible because if private Disruptor
    ...
    public static class Builder {
        ...
        protected final List<MessageHandlerInterceptor<? super CommandMessage<?>>> invokerInterceptors =
                new ArrayList<>();
        protected final List<MessageHandlerInterceptor<? super CommandMessage<?>>> publisherInterceptors =
                new ArrayList<>();
        protected final List<MessageDispatchInterceptor<? super CommandMessage<?>>> dispatchInterceptors =
                new ArrayList<>();
        protected Executor executor;
        protected boolean rescheduleCommandsOnCorruptState = true;
        protected long coolingDownPeriod = 1000;
        protected CommandTargetResolver commandTargetResolver = AnnotationCommandTargetResolver.builder().build();
        protected int publisherThreadCount = 1;
        ...
        protected int invokerThreadCount = 1;
        protected Cache cache = NoCache.INSTANCE;
        protected DuplicateCommandHandlerResolver duplicateCommandHandlerResolver = DuplicateCommandHandlerResolution.logAndOverride();

        protected CommandPolicyEnforcementPoint pep;
        ...
        /**
         * Sets the {@link CommandPolicyEnforcementPoint} to use @TODO...
         *
         * @param pep the CommandPolicyEnforcementPoint
         * @return the current Builder instance, for fluent interfacing
         */
        public Builder policyEnforcementPoint(CommandPolicyEnforcementPoint pep) {
        	assertNonNull(pep, "CommandPolicyEnforcementPoint may not be null");
        	this.pep = pep;
            return this;
        }
         ...
         */
        public SaplDisruptorCommandBus build() {
        	validate();
            return new SaplDisruptorCommandBus(this);
        }
    ...
    private class SaplDisruptorRepository<T> implements Repository<T> {

        private final Class<T> type;

        public SaplDisruptorRepository(Class<T> type) {
            this.type = type;
        }
        ...
        public Aggregate<T> newInstance(Callable<T> factoryMethod) throws Exception {
            return SaplCommandHandlerInvoker.<T>getRepository(type).newInstance(factoryMethod);
        }

        @Override
        public Aggregate<T> loadOrCreate(String aggregateIdentifier, Callable<T> factoryMethod) throws Exception {
            return SaplCommandHandlerInvoker.<T>getRepository(type).loadOrCreate(aggregateIdentifier, factoryMethod);
        }
        ...
        public void send(Message<?> message, ScopeDescriptor scopeDescription) throws Exception {
            ...
            try {
                ...
                // no idea how to test this branch
                throw e;
            }
        }
