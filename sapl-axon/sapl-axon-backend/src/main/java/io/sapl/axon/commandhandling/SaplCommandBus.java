 ...
 * and calls the Command Policy Enforcment Point before dispatching the command.
 ...
 *
 * It is possible that this CommandBus processes Commands asynchronously from the calling thread. 
 * By default, this CommandBus is not using asynchronously behavior. Set the Executor to use asynchronously behavior.   
 * 
 ...
 * 
 */
public class SaplCommandBus implements CommandBus {

    private static final Logger logger = LoggerFactory.getLogger(SaplCommandBus.class);
     ...
     */
	protected SaplCommandBus(Builder builder) {
     ...
     */
    @SuppressWarnings("unchecked") // Different types for DispatchInterceptors possible
	...
	protected <C, R> void handle(CommandMessage<C> command, MessageHandler<? super CommandMessage<?>> handler,
		...
		if (Objects.isNull(executor)) {
			CompletableFuture.runAsync(() -> run(command, handler, callback));
		} else {
			CompletableFuture.runAsync(() -> run(command, handler, callback), executor);
		}
	}

	private <C, R> void run(CommandMessage<C> command, MessageHandler<? super CommandMessage<?>> handler,
			CommandCallback<? super C, ? super R> callback) {
		...
		CheckedFunction<CommandMessage<?>, R> handleCommand = (CommandMessage<?> commandMessage) -> {
			if (commandMessage != null && !command.equals(commandMessage)) {
				unitOfWork.transformMessage(c -> commandMessage);
			}
			InterceptorChain chain = new DefaultInterceptorChain<>(unitOfWork, handlerInterceptors, handler);
			return (R)chain.proceed();
		};

		Callable<R> call = pep.preEnforceCommand(command, handleCommand);

		CommandResultMessage<R> resultMessage = asCommandResultMessage(
				unitOfWork.executeWithResult(call, rollbackConfiguration));
    ...
    public Registration subscribe(String commandName, MessageHandler<? super CommandMessage<?>> handler) {
        ...
        subscriptions.compute(commandName, (k, existingHandler) -> {
        	...
        	pep.gatherNecessaryHandlers(commandName, handler, null);
        	return handlerLocal;
        });
    ...
    public static class Builder {
        ...
        protected DuplicateCommandHandlerResolver duplicateCommandHandlerResolver =
        ...
        public SaplCommandBus build() {
        	validate();
			return new SaplCommandBus(this);
		}
