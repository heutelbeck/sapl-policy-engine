...
/**
 * Interface is used to scope ConstraintHandlerProviders to a specific MessagePayloadType.
...
public interface MessagePayloadTypeSupport<T> {
    ...
    default boolean supports(Message<?> message) {
        ...
        if (!getSupportedMessagePayloadType().isAssignableFrom(message.getPayloadType())) return false;
