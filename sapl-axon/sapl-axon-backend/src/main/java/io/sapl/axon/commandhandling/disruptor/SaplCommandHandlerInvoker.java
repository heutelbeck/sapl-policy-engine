...
/**
 * Component of the DisruptorCommandBus that invokes the command handler. The
 * execution is done within a Unit Of Work. If an aggregate has been pre-loaded,
 * it is set to the ThreadLocal.
 *
 */
public class SaplCommandHandlerInvoker implements EventHandler<CommandHandlingEntry>, LifecycleAware {

	private static final Logger                                 logger          = LoggerFactory
			.getLogger(SaplCommandHandlerInvoker.class);
	 ...
	 * {@code typeIdentifier} used by the CommandHandlerInvoker that is running on
	 * the current thread.
	 * <p>
	...
	/**
	 * Create an aggregate invoker instance for the given {@code segment} and
	 * {@code cache}.
	 *
	 ...
	 * @param pep       The Policy enforecement point
	 */
	...
	public void onEvent(CommandHandlingEntry entry, long sequence, boolean endOfBatch) {
		if (entry.isRecoverEntry()) {
			...
			if (Objects.isNull(entry.getMessage()) ||
	 ...
	 * {@code aggregateFactory}. The returning repository must be safe to use by
	 ...
	 * @param parameterResolverFactory  The factory used to resolve parameters on
	 *                                  command handler methods
	 * @return A Repository instance for the given aggregate
	 */
	public <T> Repository<T> createRepository(
	 ...
	 * @param <T>                       The type of aggregate created by the factory
	 * @param eventStore                The events store to load and publish events
	 * @param repositoryProvider        Provides repositories for specified
	 *                                  aggregate types
	 * @param aggregateFactory          The factory creating aggregate instances
	 * @param snapshotTriggerDefinition The trigger definition for snapshots
	 * @param parameterResolverFactory  The factory used to resolve parameters on
	 *                                  command handler methods
	 * @param handlerDefinition         The handler definition used to create
	 *                                  concrete handlers
	 * @return A Repository instance for the given aggregate
	 */
	@SuppressWarnings("unchecked") // Different types in repository because of possibly different aggregates
	...
	/**
	 * Repository implementation that is safe to use by a single
	 * CommandHandlerInvoker instance.
	 *
		...
		public Aggregate<T> load(String aggregateIdentifier) {
			...
			if (aggregateRoot == null) {
				...
				SnapshotTrigger   trigger     = snapshotTriggerDefinition
						.prepareTrigger(aggregateFactory.getAggregateType());
				...
				aggregateRoot = EventSourcedAggregate.initialize(aggregateFactory.createAggregateRoot(
						aggregateIdentifier, eventStream.peek()), aggregateModel, eventStore, repositoryProvider,
						trigger);
