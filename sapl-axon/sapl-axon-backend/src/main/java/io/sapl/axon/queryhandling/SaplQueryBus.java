...
public class SaplQueryBus implements QueryBus {
    ...
    public <Q, I, U> SubscriptionQueryResult<QueryResponseMessage<I>, SubscriptionQueryUpdateMessage<U>> subscriptionQuery(
        ...
        if ((originalUpdateResponseType == null) && query.getUpdateResponseType().matches(RecoverableResponse.class)){
            OriginalUpdateTypeRemoved originalUpdateTypeRemoved =
                    new OriginalUpdateTypeRemoved("A RecoverableQuery was received with missing correct metaData. Check if an Axon Server, if present removed them");
        ...
        Mono<QueryResponseMessage<I>> initialResult = getInitalResult(originalUpdateResponseType,query,updateHandlerRegistration);

        return getSubscriptionQueryResult(initialResult, updateHandlerRegistration);
    }

private <Q,I,U> Mono<QueryResponseMessage<I>> getInitalResult(ResponseType<?> originalUpdateResponseType,
                                                              SubscriptionQueryMessage<Q, I, U> query,
                                                              UpdateHandlerRegistration<U> updateHandlerRegistration){
        Mono<QueryResponseMessage<I>> mono;
    if (originalUpdateResponseType != null){
        Message<Q> genericMessage = new GenericMessage<>(query.getIdentifier(),query.getPayload(),query.getMetaData());
        SubscriptionQueryMessage<Q,I,?> originalSubscriptionQuery = new GenericSubscriptionQueryMessage<>(genericMessage,query.getQueryName(),query.getResponseType(),originalUpdateResponseType);
    ...
    //Cast to T, because MessageDispatchInterceptor.handle() always returns an Object of the same Type as the InputType
     ...
     */
    @Override
    public final Registration registerHandlerInterceptor(MessageHandlerInterceptor<? super QueryMessage<?, ?>> interceptor) {
    ...
    private <Q, R> List<MessageHandler<? super QueryMessage<?, ?>>> getHandlersForMessage(
        ...
        return subscriptions.computeIfAbsent(queryMessage.getQueryName(), k -> new CopyOnWriteArrayList<>())
                ...
                .map( querySubscription -> (MessageHandler<? super QueryMessage<?, ?>>) querySubscription.getQueryHandler())
    ...
    public static class Builder {
         ...
         *
         * @param saplqueryUpdateEmitter the {@link QueryUpdateEmitter} used to emits updates for the {@link
