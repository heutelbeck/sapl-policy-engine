...
/**
 * Implementation of the QueryBus that dispatches queries to the handlers within
 * the JVM. Any timeouts are ignored by this implementation, as handlers are
 * considered to answer immediately.
 * <p>
 * In case multiple handlers are registered for the same query and response
 * type, the {@link #query(QueryMessage)} method will invoke one of these
 * handlers. Which one is unspecified.
 *
 */
@SuppressWarnings("deprecation") // Inherited from Axon
public class SaplQueryBus implements QueryBus {

	private static final Logger logger = LoggerFactory.getLogger(SaplQueryBus.class);
	...
	/**
	 * Instantiate a {@link SimpleQueryBus} based on the fields contained in the
	 * {@link Builder}.
	 * 
	 * @param messageMonitor     a message monitor
	 * @param transactionManager a transaction manager
	 * @param errorHandler       an error handler
	 * @param queryUpdateEmitter a query update emitter
	 */
	protected SaplQueryBus(MessageMonitor<? super QueryMessage<?, ?>> messageMonitor,
		...
		if (transactionManager != NoTransactionManager.INSTANCE) {
	 ...
	 * {@link TransactionManager} to {@link NoTransactionManager},
	 * {@link QueryInvocationErrorHandler} to
	...
	public <R> Registration subscribe(
		...
		CopyOnWriteArrayList<QuerySubscription<?>> handlers          = subscriptions.computeIfAbsent(queryName,
	...
	public <Q, R> CompletableFuture<QueryResponseMessage<R>> query(QueryMessage<Q, R> query) {
		...
		try {
			...
			while (!invocationSuccess && handlerIterator.hasNext()) {
				...
				DefaultUnitOfWork<QueryMessage<Q, R>>                     uow           = DefaultUnitOfWork
						.startAndGet(interceptedQuery);
				ResultMessage<CompletableFuture<QueryResponseMessage<R>>> resultMessage = interceptAndInvoke(uow,
	...
	public <Q, R> Stream<QueryResponseMessage<R>> scatterGather(QueryMessage<Q, R> query, long timeout, TimeUnit unit) {
		...
		return handlers.stream()
				.map(handler -> {
					...
					ResultMessage<CompletableFuture<QueryResponseMessage<R>>> resultMessage = interceptAndInvoke(
							DefaultUnitOfWork.startAndGet(interceptedQuery),
							handler);
	 ...
	 *             {{@link #subscriptionQuery(SubscriptionQueryMessage, int)}
	 */
	@Override
	@Deprecated
	public <Q, I, U> SubscriptionQueryResult<QueryResponseMessage<I>, SubscriptionQueryUpdateMessage<U>>
			subscriptionQuery(
	...
	public <Q, I, U> SubscriptionQueryResult<QueryResponseMessage<I>, SubscriptionQueryUpdateMessage<U>>
		...
		if ((originalUpdateResponseType == null) && query.getUpdateResponseType().matches(RecoverableResponse.class)) {
			OriginalUpdateTypeRemoved originalUpdateTypeRemoved = new OriginalUpdateTypeRemoved(
					"A RecoverableQuery was received with missing correct metaData. Check if an Axon Server, if present removed them");

			return new DefaultSubscriptionQueryResult<>(
					Mono.error(originalUpdateTypeRemoved), Flux.error(originalUpdateTypeRemoved), () -> true);
		}

		UpdateHandlerRegistration<U> updateHandlerRegistration = queryUpdateEmitter.registerUpdateHandler(query,
				updateBufferSize);

		Mono<QueryResponseMessage<I>> initialResult = getInitalResult(originalUpdateResponseType, query,
				updateHandlerRegistration);

		return getSubscriptionQueryResult(initialResult, updateHandlerRegistration);
	}

	private <Q, I, U> Mono<QueryResponseMessage<I>> getInitalResult(
			ResponseType<?> originalUpdateResponseType,
			SubscriptionQueryMessage<Q, I, U> query,
			UpdateHandlerRegistration<U> updateHandlerRegistration) {
		Mono<QueryResponseMessage<I>> mono;
		if (originalUpdateResponseType != null) {
			Message<Q>                        genericMessage            = new GenericMessage<>(query.getIdentifier(),
					query.getPayload(), query.getMetaData());
			SubscriptionQueryMessage<Q, I, ?> originalSubscriptionQuery = new GenericSubscriptionQueryMessage<>(
					genericMessage, query.getQueryName(), query.getResponseType(), originalUpdateResponseType);
			mono = Mono.fromFuture(() -> query(originalSubscriptionQuery));
		} else {
			mono = Mono.fromFuture(() -> query(query));
		}
		return mono.doOnError(error -> {
			logger.error(format(
					"An error happened while trying to report an initial result. Query: %s",
					query), error);
			updateHandlerRegistration.getRegistration().close();
		});
	}

	private <I, U> DefaultSubscriptionQueryResult<QueryResponseMessage<I>, SubscriptionQueryUpdateMessage<U>>
	...
	private <R> CompletableFuture<QueryResponseMessage<R>> buildCompletableFuture(
	...
	// Cast to T, because MessageDispatchInterceptor.handle() always returns an
	// Object of the same Type as the InputType
	...
	/**
	 * Returns the subscriptions for this query bus. While the returned map is
	 * unmodifiable, it may or may not reflect changes made to the subscriptions
	...
	/**
	 * Registers an interceptor that is used to intercept Queries before they are
	 * passed to their respective handlers. The interceptor is invoked separately
	 * for each handler instance (in a separate unit of work).
	 *
	 * @param interceptor the interceptor to invoke before passing a Query to the
	 *                    handler
	 * @return handle to unregister the interceptor
	 */
	@Override
	public final Registration
	...
	/**
	 * Registers an interceptor that intercepts Queries as they are sent. Each
	 * interceptor is called once, regardless of the type of query (point-to-point
	 * or scatter-gather) executed.
	 *
	...
	private <Q, R> List<MessageHandler<? super QueryMessage<?, ?>>> getHandlersForMessage(
		...
		return subscriptions.computeIfAbsent(queryMessage.getQueryName(), k -> new CopyOnWriteArrayList<>())
				...
				.map(querySubscription -> (MessageHandler<? super QueryMessage<?, ?>>) querySubscription
						.getQueryHandler())
	...
	public static class Builder {
		...
		private QueryInvocationErrorHandler                errorHandler       = LoggingQueryInvocationErrorHandler
				.builder()
				.logger(logger)
				.build();
		...
		/**
		 * Sets the {@link MessageMonitor} used to monitor query messages. Defaults to a
		...
		/**
		 * Sets the {@link TransactionManager} used to manage the query handling
		 * transactions. Defaults to a {@link NoTransactionManager}.
		 *
		 * @param transactionManager a {@link TransactionManager} used to manage the
		 *                           query handling transactions
		 * @return the current Builder instance, for fluent interfacing
		 */
		...
		/**
		 * Sets the {@link QueryInvocationErrorHandler} to handle exceptions during
		 * query handler invocation. Defaults to a
		 * {@link LoggingQueryInvocationErrorHandler} using this instance it's
		 * {@link Logger}.
		 *
		 * @param errorHandler a {@link QueryInvocationErrorHandler} to handle
		 *                     exceptions during query handler invocation
		 * @return the current Builder instance, for fluent interfacing
		 */
		 ...
		 * {@link SimpleQueryUpdateEmitter}.
		 *
		 * @param saplqueryUpdateEmitter the {@link QueryUpdateEmitter} used to emits
		 *                               updates for the
		 *                               {@link QueryBus#subscriptionQuery(SubscriptionQueryMessage)}
		 * @return the current Builder instance, for fluent interfacing
		 */

		public Builder queryUpdateEmitter(SaplQueryUpdateEmitter saplqueryUpdateEmitter) {
			assertNonNull(saplqueryUpdateEmitter, "QueryUpdateEmitter may not be null");
			this.saplqueryUpdateEmitter = saplqueryUpdateEmitter;
			return this;
		}
		...
		public SaplQueryBus build() {
			validate();
			return new SaplQueryBus(messageMonitor, transactionManager, errorHandler, saplqueryUpdateEmitter);
		}
		...
		protected void validate() throws AxonConfigurationException {
			BuilderUtils.assertNonNull(this.saplqueryUpdateEmitter, "A SaplQueryUpdateEmitter is a hard requirement");
		}
	}
}
