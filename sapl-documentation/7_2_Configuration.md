...
| `oauth.pdp-id-claim` | `String` | `"sapl_pdp_id"` | JWT claim carrying the PDP identifier for tenant routing. Accepts a top-level claim name or a dot-separated path into nested claims, for example `resource_access.sapl.tenant`. An exact top-level claim wins over path interpretation, so claim names containing literal dots keep working. A malformed value (blank, or empty path segments) refuses startup. |
| `oauth.audiences` | `List<String>` | `[]` (not checked) | Audience access gate. When non-empty, a token must carry at least one of the listed values in its `aud` claim or it is rejected. Without it, any valid token from the issuer grants PDP access, so set it whenever the IdP is shared with other services and mints per-service audiences. |
| `oauth.required-scopes` | `List<String>` | `[]` (not checked) | Scope access gate. When non-empty, a token must carry at least one of the listed scopes (space-delimited `scope` claim or `scp` array) or it is rejected. The complement to `audiences` for IdPs where per-service audiences are impractical, for example a Keycloak client scope such as `sapl:pdp`. Blank entries refuse startup. |
...
```shell
docker run -e IO_SAPL_NODE_ALLOWNOAUTH=true ghcr.io/heutelbeck/sapl-node:4.2.0-SNAPSHOT
```
