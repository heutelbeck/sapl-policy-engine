...
| `keep-alive` | `long` | `15` (shipped) / `0` (code fallback) | Seconds between SSE keep-alive frames on idle streaming connections. Prevents proxies and firewalls from dropping inactive connections. `0` disables keep-alive. The bundled `application.yml` ships with `15`; if the property is absent the code fallback is `0`. See [Reverse Proxy Configuration](../7_6_Security/#reverse-proxy-configuration). |
