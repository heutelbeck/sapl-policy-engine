...
1. **Streaming**: The `then` block uses the mock ID to emit new values mid-test (see [Streaming Tests](../9_7_StreamingTests/)).
2. **Verification**: The `verify` block can reference mocks by their attribute name to check call counts (see [Verification](../9_8_Verification/)).
