...
Optionally, an attribute finder may be supplied with a list of parameters: `x.<finder.name(p1,p2,...)>`. Also, here nesting is possible. Thus `x.<finder.name(p1.<finder.name2>,p2,...)>` is a legal construct.

Further, attribute finders may be used without any leading value `<finder.name(p1,p2,...)>`. These are called environment attributes.

The way to ready a statement with an attribute finder is as follows. For `subject.<groups.membership("studygroup")>` you would say "get the attribute `group.membership` with parameter `"studygroup"` of subject".
...
Version 2.0.0 of the SAPL policy engine introduces a framework to test SAPL policies. This framework supports unit tests of a single SAPL document or policy integration test of all of your SAPL policies via the PDP interface.
...
The following https://github.com/heutelbeck/sapl-server-lt-gitops-example[Repo] is showcasing a deployment pipeline with SAPL policy tests in a https://www.weave.works/blog/gitops-operations-by-pull-request[GitOps]-Style for the headless SAPL-Server-LT.
...
[source,java]
----
.givenFunction("corp.subjectConverter", Val.of("ROLE_ADMIN"), 
	whenParameters(is(Val.of("USER")), is(Val.of("nikolai"))))
----
...
* pass an `AuthorizationSubscription` created via it's factory methods
+
[source,java]
----
...
----
* use Hamcrest Matcher prodivded by the sapl-hamcrest modul to express complex expectations on the decision, obligations, advices or resources of the `AuthorizationDecision`
+
[source,java]
----
.expect(
...
----
+
All avaliable `Matcher<AuthorizationDecision>` can be find https://github.com/heutelbeck/sapl-policy-engine/blob/master/sapl-hamcrest/src/main/java/io/sapl/hamcrest/Matchers.java#L104[here]
...
At the end of every test case definition you call the `verify` method to evaluate the policy/policies and verify the expectations.
...
* *policyPath*: Defines the path on the classpath to the folder containing the policies under test. Specify the same path used in your `SaplIntegrationTestFixture` or the parent folder of the path to your SAPL documents in the `SaplUnitTestFixture` (defaultValue = policies).
...
------
include::sapl-grammar.ebnf[SAPL-Grammar]
------