...
A SAPL policy consists of optional *imports*, optional *schemas* for authorization subscription elements,  a *name*, an *entitlement* specification, an optional *target expression*, an optional *body* with one or more statements, and optional sections for *obligation*, *advice*, and *transformation*. An example of a simple policy is:
