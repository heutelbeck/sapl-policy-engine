...
Version 2.0.0 of the SAPL policy introduces a framework to test SAPL policies. This framework supports unit tests of a single SAPL document or policy integration test of all of your SAPL policies via the PDP interface.
...
----
	...
	void setUp() {
		fixture = new SaplIntegrationTestFixture("policiesIT")
			.withPDPPolicyCombiningAlgorithm(PolicyDocumentCombiningAlgorithm.PERMIT_UNLESS_DENY);
	}
----
...
...
	...
	.expect(decision)
----
* use a Predicate-Callback to manually define checks
+
[source,java]
----
