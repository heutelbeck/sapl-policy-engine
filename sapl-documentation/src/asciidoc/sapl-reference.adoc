...
. In your application, create a new `EmbeddedPolicyDecisionPoint` (the constructor declares multiple Exceptions which must be handled). The argument `"file:C:/path/to/policies"` is a string specifying the directory which contains all policy sets and policies, as well as the configuration `pdp.json`.
+
[source,Java,linenums]
----
EmbeddedPolicyDecisionPoint pdp = new EmbeddedPolicyDecisionPoint("file:C:/path/to/policies");
----
