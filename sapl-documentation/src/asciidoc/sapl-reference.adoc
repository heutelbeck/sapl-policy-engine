...
The SAPL response to the authorization request is a JSON object as well. It contains the attributes `decision`, `resource`, `obligation` and `advice`. For the introductory sample request with the preceding policy a SAPL response would look as follows:
...
.filter.replace(Expression=null)
Replaces an attribute or an element by the result of the evaluation of `Expression`. By default, the value is replaced by `null`.
