...
AuthSubscription authSubscription = new AuthSubscription(JSON.textNode("admin"),
    JSON.textNode("an_action"), JSON.textNode("a_resource"), null);

Flux<AuthDecision> authDecisions = pdp.decide(authSubscription);
authDecisions.subscribe(authDecision -> System.out.println(authDecision.getDecision()));
----
