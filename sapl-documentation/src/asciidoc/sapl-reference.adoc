...
A typical scenario for the application of SAPL would be a subject (e.g., a user or system) attempting to take an action (e.g., read or cancel an order) on a protected resource (e.g., a domain object of an application or a file). The subject makes a subscription request to the system (e.g., an application) for executing the action on the resource. The system implements a *policy enforcement point (PEP)* protecting its resources. The PEP collects information about the subject, action, resource, and potential other relevant data in an authorization subscription request and sends it to a *policy decision point (PDP)* which checks SAPL policies to decide whether access to the resource should be granted as requested. This decision is packed in an authorization decision object and sent back to the PEP which accordingly either grants access or denies access to the resource depending on the decision. All data sources for the decision are subscribed to by the PDP and new decisions are sent to the PEP whenever indicated by the policies and data sources.
...
* *Separation of Concerns*. Applying SAPL to your domain model is relieved from modeling many aspects of access control. SAPL favors configuration at run-time over implementation and re-deployment of your applications.
...
* *Designed for session and data stream-based applications*. SAPL is designed to offer low-latency authorization for interactive applications and data streams.
...
* *Supports Multi-Subscriptions*. SAPL allows bundling multiple authorization subscriptions into one multi-subscription thus further reducing connection time and latency.
        
The following sections will explain the basic concepts of SAPL policies and show how to easily integrate SAPL into a Java application. Afterward, the different parts of SAPL are explained in more detail.
...
<2> declares that if the resource is a string starting with `https://medical.org/api/patients` (using the regular expression operator `=~`) and the conditions of the `where` clause applies, the subject will be granted access to the resource. Note that the `where` clause is only evaluated if the condition of the target expression evaluates to `true`.
...
In many use cases, all required information for making a decision may be contained in the authorization subscription object. However, the PEP is usually not aware of the specifics of the access policies and may not have access to all information required for making the decision. In this case, the PDP can access external attributes. The following example shows how attribute access is expressed in SAPL.
...
. In your application, create a new `EmbeddedPolicyDecisionPoint`. The argument `"~/sapl"` specifies the directory that contains the configuration file `pdp.json` and all policies.
+
...
The architecture of the SAPL policy engine is following the terminology defined by https://tools.ietf.org/html/rfc2904[RFC2904 "AAA Authorization Framework"]. 
...
A SAPL authorization subscription contains attributes with the names `subject`, `resource`, `action`, and `environment`. Each attribute value can be any JSON value (i.e., an object, an array,ber, a string, `true`, `false` or `null`).
...
A PDP to be used as a network service has to implement some HTTP endpoints. 
All of them accept `POST` requests and `application/json`. They produce `application/x-ndjson` as https://www.w3.org/TR/eventsource/[Server-Sent Events (SSE)]. A PDP server must be accessed over encrypted TLS connections. All connections should be authenticated. The means of authentications are left open for the organization deploying the PDP to decide, or do be defined by a specific server implementation. All endpoints should be located under a shared base URL, e.g., `https://pdp.sapl.io/api/pdp/`.
...
* SAPL Server CE: This community edition (CE) PDP server implementation uses a relational database (MariaDB) for persistence configuration and offers a convenient graphical Web interface to manage policies, configuration, and clients. The UI includes SAPL specific text editors with syntax highlighting and auto-completion features. The server is available as a docker container. Documentation: https://github.com/heutelbeck/sapl-policy-engine/tree/master/sapl-server-ce
...
The expression can make use of any element of the SAPL expression language, especially of attribute finder steps that are not allowed in the target expression.
...
The expression can make use of any element of the SAPL expression language, especially of attribute finder steps that are not allowed in the target expression. Conditions in the policy body are used to further limit the applicability of a policy. 
...
The keyword `set` is followed by the policy set name. The name is a string _identifying_ the policy set. It has to be unique within all policy sets and policies.
...
When evaluating a function expression, the expressions representing the function call arguments are evaluated first. Afterwards the results are passed to the function as arguments. The expression evaluates to the function's return value.
...
Assuming `exp1` and `exp2` are expressions evaluating to numbers, the following operators can be applied. All of them evaluate tober.
...
| `object.array2[(3+1)]`
|`5`
|*Expression step* that evaluates tober (index) - can also evaluate to an attribute name
...
Returns the value of an attribute with a key or an array item with an index specified by an expression. `Expression` must evaluate to a string orber. If `Expression` evaluates to a string, the selection can only be applied to an object. If `Expression` evaluates tober, the selection can only be applied to an array.
...
SAPL provides syntax elements for having a value pass *filters* and thereby modifying it. 
...
[NOTE]
Assuming a filter function `roundto` should round a value to the closest multiple of a given number, e.g., `207 |- roundto(100)` should return `200`. In its definition the function needs two formal parameters, the first one is reserved for the original value and the second one for the number to round to.

==== Subtemplate

It is possible to define a subtemplate for an array to replace each item of the array by this subtemplate. A subtemplate component is an optional part of a basic expression.
...
Finally in case the decision is `PERMIT` and there is a `transform` statement, the transformed resource is added to the authorization decision. Additionally, there might be an obligation and advice contained in the policies which has to be added to the authorization decision.
...
There are two layers with possibly multiple decisions which finally need to be consolidated into a single decision:
...
[NOTE]
Transformation uncertainty cannot not occur using the `only-one-applicable` combining algorithm.
...
Functions can be used within SAPL expressions (basic function expressions). A function takesber of inputs (called _arguments_) and returns an output value. 
...
SAPL will comes with a standard function library providing some basic functions.

=== Custom Function Libraries

For a more in-depth look at the process of creating a custom function library, please refer to the demo project. It will walk you through the entire process and contains extensive examples: https://github.com/heutelbeck/sapl-demos/tree/master/sapl-demo-extension .
...
The way to read a statement with an attribute finder is as follows. For `subject.<groups.membership("studygroup")>` you would say "get the attribute `group.membership` with parameter `"studygroup"` of subject".
...
For a more in-depth look at the process of creating a custom PIP, please refer to the demo project. It will walk you through the entire process and contains extensive examples: https://github.com/heutelbeck/sapl-demos/tree/master/sapl-demo-extension .

Attribute finders are functions which potentially take a left-hand argument (i.e., the Object of which the attribute is to be determined), a `Map` of variables defined in the current evaluation scope and an optional list of incoming parameter streams. 

In SAPL a Policy Information Point is an instance of a class which supplies a set of such functions. To declare functions and classes to be attributes or PIPs, SAPL uses annotations. Each PIP class has to be known to the PDP. The embedded PDP provides an `AnnotationAttributeContext` which takes arbitrary Java classes as PIPs. To be recognized as a PIP, the class has to be annotated with `@PolicyInformationPoint`. The optional annotation attribute `name` contains the PIP's name as it will be available in SAPL policies. If the attribute is missing, the name of the Java class is used. The optional annotation attribute `description` contains a string describing the PIP for documentation purposes.
+
[source,java]
...
The individual attributes supplied by the PIP are identified by adding the annotation `@Attribute`. An optional annotation attribute `name` can contain a name for the attribute. The attribute has to be a string containing an identifier. By default, the name of the function will be used. The annotation attribute `docs` can contain a string describing the attribute.
+
[source,java]
@Attribute(name = "profile", docs = "returns user profile as JSON")
public Flux<Val> userprofile(@Number leftHandObjectOfTheAttribute, Map<String, JsonNode> variablesInCurrentEvaluationContext, Flux<Val> parameter1, Flux<Val> parameter2) {
    ...
}
+

== Testing your SAPL policies

The SAPL policy engine provides a framework to test SAPL policies. This framework supports unit tests of a single SAPL document or policy integration tests of all of your SAPL policies via the PDP interface.

=== Usage scenarios

With the SAPL test framework you can test SAPL policies no matter if you use SAPL via a embedded PDP in your application or via a central SAPL server.

==== Embedded PDP

If you're using a embedded PDP in your application, you can treat your SAPL policy tests like your "normal" unit and integration tests. Organize them next to these tests and execute them identically via the Maven lifecycle on your local workstation or in your CI pipeline.

==== SAPL-Server

The following the repository https://github.com/heutelbeck/sapl-server-lt-gitops-example[GitOps Demo] is showcasing a deployment pipeline with SAPL policy tests in a https://www.weave.works/blog/gitops-operations-by-pull-request[GitOps]-Style for the headless SAPL-Server-LT.
Here every change to the policies is introduced via a pull request on the main branch. For every pull request the CI pipeline executes the policy tests and breaks the pipeline run if policy tests are failing.
If a pull request is merged on the main branch, the policies are automatically synced to the SAPL-Server-LT instance.

Currently there is no support for writing SAPL test cases via the SAPL DSL - only in Java. So it's not possible to use the SAPL test framework when deploying SAPL-Server-Implementations with GUI-based PAP (SAPL-Server-CE or SAPL-Server-EE).

=== Unit-Tests

The JUnit-Test-Framework is used for execution of the SAPL unit test cases. You create and setup a `SaplUnitTestFixture` for example in the `@BeforeStep` of a JUnit test case.

Here you define the name of the SAPL document under test or the path to this file. In addition you can register PIPs and FunctionLibrarys to be used during test execution by providing these objects to the `SaplUnitTestFixture`.
...
Instead of testing a single SAPL document, you can test all policies together via the PDP interface just like when you're using a PDP embedded in your application or a SAPL server implementation.

For this you create a `SaplIntegrationTestFixture` again for example in the `@BeforeStep` of your JUnit test case. Likewise you can register PIPs and FunctionLibraries in addition to a combining algorithm.
...
Starting with constructTestCaseWithMocks() or constructTestCase() called on your fixture, the test case definition process is started at the Given-Step or the When-Step.

==== Given-Step

*Mocking of functions*:

* via the `givenFunction` methods you can mock a function returning the `Val` specified in the method parameters for every call. 
...
** a single value only returned when the paramters of the function call match some expectations
+
[source,java]
----
...
* via the `givenFunctionOnce` methods you can specify a `Val` or multiple `Val`-Objects which are emitted once (in a sequence) when this mocked function is called
...
* via the `givenAttribute` methods you can mock an attribute
...
** to return a sequence of `Val` in an interval of `Duration`. Do not forget to call the `withVirtualTime` method to use the virtual time feature of Project Reactor
+
[source,java]
----
.withVirtualTime()
...
----
[WARNING]
The virtual time feature can be used with real time-based PIPs registrated at the fixture level. But be aware that virtual time is "no silver bullet" to cite the https://projectreactor.io/docs/core/release/reference/#_manipulating_time[Project Reactor Reference Guide]. It says further that "[v]irtual time also gets very limited with infinite sequences, which might hog the thread on which both the sequence and its verification run."

** to mark an attribute to be mocked and specify return values in a sequence next to your expectations
+
[source,java]
----
...
You can check out further mock types or overloaded methods https://github.com/heutelbeck/sapl-policy-engine/blob/master/sapl-test/src/main/java/io/sapl/test/steps/GivenStep.java[here].

==== When-Step

In the next step you define the `AuthorizationSubscription` for the policy evaluation.
...
Define the expected `AuthorizationDecision` in this step.
...
----
* use Hamcrest Matcher provided by the sapl-hamcrest modul to express complex expectations on the decision, obligations, advice or resources of the `AuthorizationDecision`
+
[source,java]
----
.expect(
...
----
+
All avaliable `Matcher<AuthorizationDecision>` can be found https://github.com/heutelbeck/sapl-policy-engine/blob/master/sapl-hamcrest/src/main/java/io/sapl/hamcrest/Matchers.java#L104[here]

All of these methods come with additional methods (for example `expectNextPermit`) to define multiple expectations for testing of stream-based policies.
[source,java]
----
.expectNextPermit(3)
----
...
You can check out all available methods https://github.com/heutelbeck/sapl-policy-engine/blob/master/sapl-test/src/main/java/io/sapl/test/steps/ExpectStep.jav[here]. 

==== Verify-Step

At the end of every test case definition, you must call the `verify()` method to evaluate the policy/policies and verify the expectations.

=== Examples

In the following you can find a full and minimal example of a SAPL unit test.
[source,java]
----
...
For measuring the policy code coverage of your SAPL policies you can use the sapl-maven-plugin to analyze the coverage and generate reports in various formats.

Currently three coverage criteria are supported:

- *PolicySet Hit Coverage*: Measures the percentage of PolicySets which were at least once applicable to an `AuthorizationSubscription` in your tests.

- *Policy Hit Coverage*: Measures the percentage of Policies which were at least once applicable to an `AuthorizationSubscription` in your tests.

- *Condition Hit Coverage*: Measures the percentage of conditions  evaluated to true or false during your tests. Twice the number of conditions is taken and compared with the number of positively and negatively evaluated conditions.

Simply add the following configuration to your `pom.xml`
[source,xml]
----
<plugin>
	<groupId>io.sapl</groupId>
...
The plugin can be configured via the following paramters:

* *coverageEnabled*: When set to false, this parameter disables the execution of the sapl-maven-plugin (defaultValue = true).
* *policyPath*: Defines the path in the classpath to the folder containing the policies under test. Specify the same path used in your `SaplIntegrationTestFixture` or the parent folder of the path to your SAPL documents in the `SaplUnitTestFixture` (defaultValue = policies).
* *outputDir*: Set this parameter to the path where generated reports should be written to (per default the maven build output directory is used).
* *policySetHitRatio*: A value between 0 - 100 to define the ratio of PolicySets your tests should cover. If this ratio isn't fulfilled the sapl-maven-plugin is going to stop the maven lifecycle. (defaultValue = 0)
* *policyHitRatio*: A value between 0 - 100 to define the ratio of Policies your tests should cover. If this ratio isn't fulfilled the sapl-maven-plugin is going to stop the maven lifecycle (defaultValue = 0).
* *policyConditionHitRatio*: A value between 0 - 100 to define the ratio of condition results your tests should cover. If this ratio isn't fulfilled the sapl-maven-plugin is going to stop the maven lifecycle (defaultValue = 0).
* *enableSonarReport*: When set to true a coverage report with the https://monteverdi.informatik.uni-freiburg.de/sonar/documentation/analysis/generic-test/[Sonarqube Generic Coverage Format] is generated. This format is currently not useable as SonarQube does not import generic coverage data for languages unknown to SonarQube. Currently there is no SonarQube Lanaguage Plugin for SAPL (defaultValue = false).
* *enableHtmlReport*: When set to true a HTML coverage report is created. This report is similar to JaCoCo reports showing colorized line coverage and number of covered branches for conditions in a line. The path to the `index.html` on the filesystem is printed in the maven log. Terminals like Powershell allow you to click on these paths and open the report directly in your browser (defaultValue = true).
