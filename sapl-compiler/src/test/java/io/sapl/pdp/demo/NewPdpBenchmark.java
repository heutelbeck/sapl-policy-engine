...
/**
 * Performance benchmark for the NEW Value-based Policy Decision Point.
 * <p>
 * This benchmark uses the new {@link PolicyDecisionPointBuilder} and
 * {@link io.sapl.pdp.DynamicPolicyDecisionPoint} to measure throughput
 * and latency, enabling direct comparison with the old Val-based PDP.
 * <p>
 * Benchmark modes:
 * <ul>
 ...
 * </ul>
 * <p>
 * Usage: java NewPdpBenchmark [mode] [iterations] [warmup]
 */
public final class NewPdpBenchmark {
    ...
    /**
     * Multiple policies simulating a realistic policy set - identical to
     * EmbeddedPdpBenchmark for fair comparison.
     */
    private static final String[] POLICIES = { "policy \"admin_full_access\" permit where subject.role == \"admin\";",
    ...
    public NewPdpBenchmark(int iterations, int warmupRounds) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var benchmark = new NewPdpBenchmark(iterations, warmup);
        ...
        case "pure"     -> benchmark.runPureScalingAnalysis();
        case "minimal"  -> benchmark.runMinimalScalingAnalysis();
        case "latency"  -> benchmark.runLatencyUnderLoad();
        case "all"      -> benchmark.runAll();
        default         -> {
            System.err.println("Unknown mode: " + mode);
            System.err.println("Available modes: single, reactor, executor, scaling, pure, minimal, latency, all");
            System.exit(1);
        }
        }
    }
    ...
    private void initialize() throws Exception {
        printHeader("NEW PDP BENCHMARK SUITE (Value-based)");
        System.out.printf("Cores: %d | Iterations: %,d | Warmup: %,d%n", cores, iterations, warmupRounds);
        System.out.printf("Policies: %d | Combining: DENY_OVERRIDES%n%n", POLICIES.length);

        System.out.println("Initializing NEW PDP with multiple policies...");
        ...
        var configuration         = new PDPConfiguration("default", "benchmark-config",
                CombiningAlgorithm.DENY_OVERRIDES, List.of(POLICIES), Map.of());
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            var key      = decision != null ? decision.decision().name() : "NULL";
    ...
    /**
     * Scaling analysis with chunked work distribution.
     */
    ...
    /**
     * Scaling analysis using decidePure() - bypasses Reactor subscription overhead.
     * This demonstrates the performance improvement from lock-free configuration
     * access.
     */
    private void runPureScalingAnalysis() throws Exception {
        printHeader("SCALING ANALYSIS - PURE (Lock-free config access)");

        var testIterations    = Math.min(iterations, 500_000);
        var concurrencyLevels = new int[] { 1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 14, 16, 20, 24, 32, 40, 48 };

        // Global warmup using decidePure
        System.out.println("Global warmup (single-threaded, decidePure)...");
        for (int i = 0; i < warmupRounds; i++) {
            BLACKHOLE.set(pdp.decidePure(subscriptions[i % subscriptions.length]));
        }
        System.out.println("Warmup complete.\n");

        System.out.println("Testing throughput with decidePure() (lock-free config access)...\n");
        System.out.printf("%-12s %15s %15s %12s%n", "Concurrency", "Throughput", "Per-Thread", "Speedup");
        System.out.println("-".repeat(60));
    ...
    /**
     * Scaling analysis using decidePureMinimal() - bypasses both Reactor and UUID
     * generation.
     * This isolates whether UUID.randomUUID() is the contention source.
     */
    private void runMinimalScalingAnalysis() throws Exception {
        printHeader("SCALING ANALYSIS - MINIMAL (No UUID generation)");

        var testIterations    = Math.min(iterations, 500_000);
        var concurrencyLevels = new int[] { 1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 14, 16, 20, 24, 32, 40, 48 };

        // Global warmup using decidePureMinimal
        System.out.println("Global warmup (single-threaded, decidePureMinimal)...");
        for (int i = 0; i < warmupRounds; i++) {
            BLACKHOLE.set(pdp.decidePureMinimal(subscriptions[i % subscriptions.length]));
        }
        System.out.println("Warmup complete.\n");

        System.out.println("Testing throughput with decidePureMinimal() (no UUID generation)...\n");
        System.out.printf("%-12s %15s %15s %12s%n", "Concurrency", "Throughput", "Per-Thread", "Speedup");
        System.out.println("-".repeat(60));
    ...
    private AuthorizationSubscription createSubscription(int seed) {
        ...
        return new AuthorizationSubscription(ValueJsonMarshaller.json(subjectString), Value.of(action),
                ValueJsonMarshaller.json(resourceString), Value.UNDEFINED);
    }
