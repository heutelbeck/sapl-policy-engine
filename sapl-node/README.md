...
The executable JAR is at `sapl-node/target/sapl-node-4.1.1.jar`.
...
spec:
  template:
    spec:
      containers:
        - name: sapl
          image: ghcr.io/heutelbeck/sapl-node:4.1.1
          ports:
            - containerPort: 8080
          livenessProbe:
            httpGet:
...
./run-sapl4.sh "java -jar ../target/sapl-node-4.1.1.jar" /tmp/bench-jvm
./run-sapl4.sh ../target/sapl /tmp/bench-native quick
./run-all.sh "java -jar ../target/sapl-node-4.1.1.jar" /tmp/bench-full standard
```
