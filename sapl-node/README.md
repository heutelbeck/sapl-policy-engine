...
The executable JAR is at `sapl-node/target/sapl-node-4.1.2.jar`.
...
./run-sapl4.sh "java -jar ../target/sapl-node-4.1.2.jar" /tmp/bench-jvm
./run-sapl4.sh ../target/sapl /tmp/bench-native quick
./run-all.sh "java -jar ../target/sapl-node-4.1.2.jar" /tmp/bench-full standard
```
