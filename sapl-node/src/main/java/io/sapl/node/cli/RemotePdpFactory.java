...
/**
 * Creates a {@link PolicyDecisionPoint} for remote HTTP connections from
 * benchmark context parameters. Uses the same builder pattern as
 * {@link PdpSetup#openRemote}.
 */
@UtilityClass
class RemotePdpFactory {

    static final String ERROR_BASIC_AUTH_FORMAT = "Error: --basic-auth must be in format 'user:password'.";

    static PolicyDecisionPoint create(BenchmarkContext ctx) throws SSLException {
